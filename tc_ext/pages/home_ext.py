...
def _tc_ext_data_root() -> str:
    """Retorna o caminho raiz dos dados TC Ext, compatível com local e cloud."""
    return os.path.join(str(_get_data_root()), "TC_Ext")
...
def obter_data_atualizacao_dados():
    ...
    try:
        ...
        _ext_root = _tc_ext_data_root()
        arquivos_dados = [
            # Caminhos do histórico consolidado
            os.path.join(_ext_root, "historico_consolidado", "df_final_historico.parquet"),
            os.path.join(_ext_root, "historico_consolidado", "df_vol_historico.parquet"),
            os.path.join(_ext_root, "historico_consolidado", "BUD", "df_final_historico_BUD.parquet"),
        ]
        
        # Também tentar buscar em pastas de anos recentes
        pasta_dados = _ext_root
...
def _hist_consolidado(*parts: str) -> str:
    """Caminho para arquivo em TC_Ext/historico_consolidado/."""
    return os.path.join(_tc_ext_data_root(), "historico_consolidado", *parts)
...
def get_budget_oficinas_opcoes(ano_selecionado_param):
    ...
    try:
        caminho_budget = _hist_consolidado("BUD", "df_final_historico_BUD.parquet")
...
def get_budget_volume_oficinas_opcoes(ano_selecionado_param):
    ...
    try:
        caminho_budget_vol = _hist_consolidado("BUD", "df_vol_historico_BUD.parquet")
...
st.session_state.filtro_ano_tc_ext = str(ano_selecionado) if ano_selecionado != 'Todos' else 'Todos'
st.session_state.moeda_selecionada = _map_labels_moeda.get(moeda_codigo, '🇧🇷 R$')
...
def load_data(ano_selecionado_param):
    ...
    try:
        ...
        caminho_historico = _hist_consolidado("df_final_historico.parquet")
...
if is_main_page:
    ...
    if 'Período' in df_filtrado.columns:
        ...
        # Guardar cópia ANTES do filtro de período (para gráfico de todos os meses)
        df_antes_filtro_periodo = df_filtrado.copy()
...
def load_data(ano_selecionado_param):
    ...
    try:
        # IMPORTANTE: Sempre carregar do histórico consolidado para garantir consistência
        # Apenas aplicar filtro de ano quando necessário
        df = _load_ext_delta_or_parquet("principal", ("df_final_historico.parquet",), ano_selecionado_param)

        if df is None:
            caminho_historico = _hist_consolidado("df_final_historico.parquet")
            delta_root = os.environ.get("SCI_DELTA_ROOT", "")
            msg_delta = f" | Delta: {delta_root}/TC_Ext/<ano>/principal" if delta_root else ""
            st.error(f"❌ Arquivo de histórico consolidado não encontrado: {caminho_historico}{msg_delta}")
            st.info("💡 Execute o notebook 03 com DATASET_KEY=TC_Ext para gerar os dados")
            st.stop()
            return None
...
def load_volume_data(ano_selecionado_param):
    ...
    try:
        # IMPORTANTE: Sempre carregar do histórico consolidado para garantir consistência
        # Apenas aplicar filtro de ano quando necessário
        df = _load_ext_delta_or_parquet("volume_real", ("df_vol_historico.parquet",), ano_selecionado_param)
...
def load_budget_data(ano_selecionado_param):
    ...
    try:
        df = _load_ext_delta_or_parquet("BUD", ("BUD", "df_final_historico_BUD.parquet"), ano_selecionado_param)
...
def load_budget_volume_data(ano_selecionado_param):
    ...
    try:
        df = _load_ext_delta_or_parquet("volume_BUD", ("BUD", "df_vol_historico_BUD.parquet"), ano_selecionado_param)
...
def calcular_flex_budget(df_real, df_real_vol, df_budget, df_budget_vol, tipo_viz, tem_ano):
    ...
    - FLEX_Fixo = Custo_Fixo_Real x Variação_% x 0 = 0
    - FLEX_Variável = Custo_Variável_Real x Variação_% x 1
...
def create_period_chart(df_data, coluna, tipo_viz, df_budget=None, df_budget_vol=None, df_real_vol=None, df_real_original=None, moeda_simbolo="R$", debug=False, debug_context=""):
    ...
    try:
        ...
        # Garantir todos os 12 meses no eixo X (mesmo sem dados naquele mês)
        _meses_cap = [
            'Janeiro', 'Fevereiro', 'Março', 'Abril', 'Maio', 'Junho',
            'Julho', 'Agosto', 'Setembro', 'Outubro', 'Novembro', 'Dezembro',
        ]
        if tem_ano and coluna_periodo_grafico == 'Período_Completo':
            # Garantir 12 meses por ano: criar MultiIndex completo (Ano × Mês)
            _anos_presentes = sorted(chart_data['Ano'].dropna().unique())
            _idx_full = pd.MultiIndex.from_product(
                [_anos_presentes, _meses_cap], names=['Ano', 'Período'],
            )
            chart_data = (
                chart_data.set_index(['Ano', 'Período'])
                .reindex(_idx_full)
                .reset_index()
            )
            chart_data['Período_Completo'] = (
                chart_data['Período'].astype(str) + ' ' + chart_data['Ano'].astype(str)
            )
            ordem_periodos = chart_data['Período_Completo'].tolist()
        elif not tem_ano and coluna_periodo_grafico == 'Período':
            chart_data = chart_data.set_index('Período').reindex(_meses_cap).reset_index()
            ordem_periodos = _meses_cap
...
if is_main_page:
    ...
    # Criar df_para_grafico_periodo usando dados ANTES do filtro de período
    # para que o gráfico mostre TODOS os meses, não apenas o selecionado.
    if 'df_antes_filtro_periodo' in locals() and df_antes_filtro_periodo is not None:
        df_para_grafico_periodo = df_antes_filtro_periodo.copy()
    elif 'df_filtrado' in locals() and df_filtrado is not None:
    ...
    with tab1:
        ...
        def _render_tab1():
            ...
            if df_budget_filtrado is not None and df_budget_vol_filtrado is not None and df_volume_real_filtrado is not None:
                ...
                expand_state_key = 'home_ext_flex_expand_all'
                ...
                with ctrl_col1:
                    if st.button('Expandir tudo', key='home_ext_expandir_flex'):
                        st.session_state[expand_state_key] = True
                with ctrl_col2:
                    if st.button('Recolher tudo', key='home_ext_recolher_flex'):
                        st.session_state[expand_state_key] = False
                with ctrl_col3:
                ...
                if 'Custo' in df_budget_filtrado.columns and tem_custo_real:
                    ...
                    if df_real_tabela is None or len(df_real_tabela) == 0:
                        ...
                        if tipo_visualizacao == "CPU (Custo por Unidade)":
                            ...
                            # ðŸ”’ Governança: Fixo não flexibiliza (independente do texto vir como 'Fixo ', 'FIXO', etc.)
                            ...
                            # Usar operações vetorizadas ao invés de apply (muito mais rápido)
                            # ðŸ”’ Governança: Fixo não flexibiliza
                            mask_fixo = _mask_custo_fixo(df_tabela_flex['Custo']) if 'Custo' in df_tabela_flex.columns else pd.Series(False, index=df_tabela_flex.index)
                            df_tabela_flex['_Flex_Bud_Fixo'] = df_tabela_flex['Budget_Total_Custo'].where(mask_fixo, 0)
                            df_tabela_flex['_Flex_Bud_NaoFixo'] = (df_tabela_flex['Budget_Total_Custo'] * df_tabela_flex['_Proporcao_Volume']).where(~mask_fixo, 0)
                        ...
                        if len(df_tabela_flex) > 0:
                            ...
                            # ðŸ”Ž GOVERNANÇA (VISIBILIDADE): remover linhas totalmente zeradas/nulas apenas na EXIBIÇÃO.
                            ...
                            if modo_tabela_flex == "Fixo/Variável":
                                ...
                                for custo in ['Fixo', 'Variável']:
                                    ...
                                    if len(df_custo) > 0:
                                        ...
                                        with st.expander(f"💰 {custo} - Total: {total_custo_formatado}", expanded=expandir_flex):
                                            ...
                                            if 'Type 05' in df_custo.columns:
                                                for type05 in sorted(df_custo['Type 05'].dropna().unique()):
                                                    ...
                                                    if len(df_type05) > 0:
                                                        ...
                                                        with st.expander(f"📊 Type 05: {type05} - Total: {total_type05_formatado}", expanded=expandir_flex):
                                                                ...
                                                                for type06 in sorted(df_type05['Type 06'].dropna().unique()):
                                                                    ...
                                                                    if len(df_type06) > 0:
                                                                        ...
                                                                        if 'Account' in df_type06.columns:
                                                                            ...
                                                                            # Só exibir se houver dados após filtrar
                                                                            if len(df_type06_filtrado) > 0:
                                                                                with st.expander(f"📑 Type 06: {type06} - Total: {total_type06_formatado}", expanded=False):
    ...
    def create_oficina_chart(df_data, coluna, tipo_viz, moeda_simbolo="R$", df_budget=None, df_budget_vol=None, df_real_vol=None, df_real_original=None):
        ...
        try:
            # Robustez: garantir nomes/valores canÃ´nicos (ex.: Per\uFFFDodo -> Período)
            try:

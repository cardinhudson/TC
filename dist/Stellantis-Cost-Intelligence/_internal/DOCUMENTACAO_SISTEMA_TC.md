...
## 1.4) Mudanças recentes — Fev/2026 (Conferências + Correções Extração)

### Conferências automáticas (`executar_conferencias`)
- Nova função `executar_conferencias(ano, tipo)` em `processamento_dados_veiculos.py` — valida dados processados (parquets) contra as mesmas abas-fonte do Excel usadas pela extração.
- 6 verificações: Despesa Primária, Redis, Volume FA, Custo FA, Custo FP, Prova cruzada (DP = FA + FP).
- Real lê da aba `Sapiens` (header=1, formato longo) — mesma fonte que `fase1_sapiens()`.
- Budget lê da aba `massa primária - BDG` (melt) — mesma fonte que `fase1_voz_de_custo()`.
- FA e FP são valores **calculados** (não lidos do Excel): validados por prova cruzada linha a linha + comparação contra abas de síntese (`massa FA/FP`).
- Integrada na página Home (substituiu ~280 linhas de validação inline) e na página Extração (executa após processar).

### Fallback BDG para Volume FA
- `fase2_volume_est_fa()` agora detecta quando a aba `Volume e EST PdR - Actual` está vazia e usa dados de `Volume e EST PdR - BDG` como fallback, marcando `_fonte_volume_fa = 'BDG'`.

### Correção na leitura de abas-fonte
- Corrigido: a conferência agora lê das **mesmas abas** que a extração usa como fonte (Sapiens para Real, massa primária para BDG), em vez de comparar contra abas de síntese (massa FA/FP - Actual) que não são fontes da pipeline.

### Build EXE — Limpeza de cache
- `build_exe.bat` agora limpa `__pycache__/` de todas as pastas antes do build e após copiar para `_internal/`, evitando bytecode stale no EXE.
- Deleta o `.spec` anterior para forçar regeneração completa.
- Passo de verificação pós-build: executa conferências automaticamente nos dados do `_internal/`.

## 1.5) Mudanças recentes — Fev/2026 (Simulador BE + Gráficos TC Veículos)
...
## 14.9 Conferências Automáticas (`executar_conferencias`)

Função: `executar_conferencias(ano, tipo)` em `processamento_dados_veiculos.py`

**Objetivo:** Validar os parquets gerados pela extração contra as mesmas abas-fonte do Excel, garantindo que os dados processados estão consistentes com a origem.

### Princípio fundamental

> A conferência lê do Excel **exatamente as mesmas abas** usadas pela extração, usando a **mesma lógica de leitura** (`_ler_aba_melt`, `_ler_sapiens`, `_ler_redis_melt`). **Nunca** se compara contra abas que não são fonte da pipeline.

### Abas-fonte por tipo

| Check | Real (fonte Excel) | Budget (fonte Excel) |
|-------|-------------------|---------------------|
| 1) Despesa Primária | `Sapiens` (header=1, long) | `massa primária - BDG` (wide, melt) |
| 2) Redis | `massa - REDIS` (wide, melt) | `massa - REDIS` (wide, melt) |
| 3) Volume FA | `Volume e EST PdR - Actual` | `Volume e EST PdR - BDG` |
| 4) Custo FA | `massa FA - Actual` (wide, melt) | `massa FA - BDG` (wide, melt) |
| 5) Custo FP | `massa FP - Actual` (wide, melt) | `massa FP - BDG` (wide, melt) |
| 6) Prova cruzada | DP = FA + FP (linha a linha) | DP = FA + FP (linha a linha) |

### Helpers internos

| Helper | Descrição |
|--------|-----------|
| `_ler_sapiens()` | Réplica de `fase1_sapiens()`: header=1, alias colunas, exclui Account='Redis', exclui `OFICINAS_INVALIDAS` |
| `_ler_aba_melt(sheet)` | Réplica de leitura wide: `_corrigir_colunas_mojibake` → `_detectar_colunas_meses` → melt → `_normalizar_periodo` → exclui `OFICINAS_INVALIDAS` |
| `_ler_redis_melt()` | `_ler_aba_melt('massa - REDIS')` + remove zeros |

### Critério de aprovação

- **✅**: diferença percentual < 0.01% (tolerância para arredondamento de rateio)
- **⚠️**: diferença percentual entre 0.01% e 1%
- **❌**: diferença percentual ≥ 1% ou prova cruzada com erros

### Por que FA/FP podem ter diferença mínima (~0.36)

Custo FA e FP são **calculados** pela extração (FA = DP × Rateio, FP = DP − FA), não lidos diretamente do Excel. As abas `massa FA/FP` do Excel são calculadas pela planilha com precisão ligeiramente diferente. Diferenças < 1 são normais e aceitáveis.

### Onde é executada

1. **Home TC** (`tc_principal/pages/home_tc.py`): botão "Executar Validação Excel × SCI" dentro do expander de validação.
2. **Extração de Dados** (`tc_principal/pages/extracao_dados_tc.py`): executa automaticamente após cada extração Real ou Budget.

### Fallback BDG para Volume FA

Quando a aba `Volume e EST PdR - Actual` está vazia (caso de anos futuros ou dados ainda não alimentados), a `fase2_volume_est_fa()` usa automaticamente `Volume e EST PdR - BDG` como fallback. O parquet fica marcado com `_fonte_volume_fa = 'BDG'` e a conferência exibe a indicação `[Fallback BDG]`.

---

## 15) Build do EXE (Empacotamento PyInstaller)

### Processo de Build

O build é feito por `build_exe.bat`, que usa `streamlit-desktop-app` + xcopy manual:

```
1. Ativa o .venv
2. Limpa builds anteriores, __pycache__, e spec antigo
3. streamlit-desktop-app build app.py --name Stellantis-Cost-Intelligence
4. xcopy de dados/, pages/, tc_core/, tc_principal/, tc_ext/, scripts .py, JSONs, imagens para _internal/
5. Remove __pycache__ do _internal/
6. Verifica conferências nos dados do _internal/
```
...
├── Stellantis-Cost-Intelligence.exe     ← entry point (chama launcher.py interno)
├── taxas_cambio.db                      ← banco de câmbio (via os.getcwd())
└── _internal/                           ← sys._MEIPASS quando frozen
    ├── app.py                           ← Streamlit app
    ├── processamento_dados*.py          ← scripts de extração
    ├── dados/TC_Principal/{ano}/        ← Excels + Parquets
    ├── tc_core/, tc_principal/, tc_ext/  ← módulos do projeto
    ├── pages/                           ← páginas Streamlit
    ├── *.json                           ← configs (versao, rateios, etc.)
    └── streamlit/, pandas/, ...         ← dependências Python
```

### Resolução de caminhos (`_ROOT`)

Todos os módulos usam o mesmo padrão:

```python
if hasattr(sys, '_MEIPASS'):
    _ROOT = sys._MEIPASS          # EXE → _internal/
else:
    _ROOT = os.path.dirname(...)  # Dev → raiz do projeto
```

Caminhos de dados: `os.path.join(_ROOT, 'dados', 'TC_Principal', str(ano), ...)`

### Armadilhas comuns do EXE

| Problema | Causa | Solução |
|----------|-------|---------|
| Dados stale no EXE | `__pycache__/` com .pyc antigos copiados pelo xcopy | Limpar `__pycache__` antes e depois do build |
| Spec reutilizado | `Stellantis-Cost-Intelligence.spec` anterior com imports antigos | Deletar spec antes do build |
| Módulo antigo no PYZ | PyInstaller empacota bytecode no PYZ archive | Limpar pasta `build/` inteira |
| Re-extração com código errado | PYZ tem prioridade sobre .py no `_internal/` | Garantir que build é feito com código atualizado |
| `taxas_cambio.db` não encontrado | Usa `os.getcwd()`, que aponta para pasta do EXE | Arquivo copiado para nível do `.exe` (fora de `_internal/`) |

### Fluxo do usuário no EXE

1. Usuário abre o EXE → `launcher.py` faz `os.chdir(sys._MEIPASS)` e inicia servidor Streamlit
2. Abre `pywebview` (desktop) ou navegador com `http://127.0.0.1:8501`
3. Usuário roda extração → dados são reprocessados e gravados no `_internal/dados/`
4. Os parquets atualizados são usados imediatamente pelo dashboard

### Regra permanente

> **Sempre que alterar código de processamento ou cálculo, refazer o build do EXE com `build_exe.bat`.** Nunca copiar apenas o `.py` para `_internal/` manualmente — o PYZ archive dentro do EXE pode sobrescrever o `.py` em runtime.

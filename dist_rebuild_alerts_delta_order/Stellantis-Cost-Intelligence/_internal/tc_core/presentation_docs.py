...
def _ui_texts(language: str) -> dict[str, str]:
    if language == "en":
        return {
            ...
            "download_pdf": "📄 Export presentation as PDF",
            "download_ppt": "📊 Export presentation as PPT",
            "tabs_script": "🎤 Script (5 min)",
            "tabs_slides": "🧩 Visual slides",
    ...
    return {
        ...
        "download_pdf": "📄 Exportar Apresentacao em PDF",
        "download_ppt": "📊 Exportar Apresentacao em PPT",
        "tabs_script": "🎤 Roteiro (5 min)",
        "tabs_slides": "🧩 Slides visuais",
...
def _pdf_draw_list(pdf, items: list[str], style: ParagraphStyle, left_in: float, top_in: float, width_in: float, gap_in: float = 0.08) -> float:
    current_top = top_in
    for item in items:
        used = _pdf_draw_paragraph(pdf, f"• {item}", style, left_in, current_top, width_in)
...
def render_presentation_section(version: str, data_atualizacao: str | None) -> None:
    ...
    with tab_slides:
        ...
        for index, slide in enumerate(slides):
            with st.expander(f"{slide.label} — {slide.title}", expanded=index == 0):
                if slide.cover:

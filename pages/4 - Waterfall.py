    ...
    if len(type06_valores) == 0:
        ...
        with tab_real:
                ...
                try:
                    ...
                    if tipo_visualizacao == "CPU (Custo por Unidade)":
                        if 'CPU' in df_visualizacao.columns:
                            col_valor = 'CPU'
                        elif 'Total' in df_filtrado.columns and 'Volume' in df_filtrado.columns:
                            col_valor = 'Total'
                        else:
                            st.error("❌ Dados insuficientes para calcular CPU.")
                            st.stop()
                    else:
                        if 'Total' in df_filtrado.columns:
                            col_valor = 'Total'
                    ...
                    if df_volume is not None and not df_volume.empty and 'Volume' in df_volume.columns:
                        ...
                        # Aplicar os mesmos filtros do sidebar (mesma lógica usada para df_filtrado_waterfall)
                        # Filtros de Oficina
                    ...
                    if df_m1.empty or df_m2.empty:
                        ...
                        if tipo_visualizacao == "CPU (Custo por Unidade)" and 'Total' in df_m1.columns and 'Volume' in df_m1.columns:
                            g1 = df_m1.groupby(chosen_dim_waterfall).agg({'Total': 'sum', 'Volume': 'sum'}).reset_index()
                            g1['CPU'] = g1['Total'] / g1['Volume'].replace(0, 1)
                            g1 = g1.set_index(chosen_dim_waterfall)['CPU']
                            
                            g2 = df_m2.groupby(chosen_dim_waterfall).agg({'Total': 'sum', 'Volume': 'sum'}).reset_index()
                            g2['CPU'] = g2['Total'] / g2['Volume'].replace(0, 1)
                            g2 = g2.set_index(chosen_dim_waterfall)['CPU']
                            
                            total_m1_all = (df_m1['Total'].sum() / df_m1['Volume'].sum()) if df_m1['Volume'].sum() > 0 else 0
                            total_m2_all = (df_m2['Total'].sum() / df_m2['Volume'].sum()) if df_m2['Volume'].sum() > 0 else 0
                        else:
                        ...
                        # Calcular Flex Mês 1 para cada categoria
                        for cat in cats_sel:
                            if cat in g1.index:
                                ...
                                flex_m1_cat = total_fixo_cat + (total_variavel_cat * proporcao_volume_cat)
                                
                                if tipo_visualizacao == "CPU (Custo por Unidade)":
                                    # Converter para CPU
                                    bud_cat = valor_m1_cat / volume_m1_cat if volume_m1_cat != 0 else 0
                                    flex_m1_cat = flex_m1_cat / volume_m2_cat if volume_m2_cat != 0 else 0
                        ...
                        try:
                            ...
                            # Base_Total = Total do mês 1 (BUD) - usar total_m1_all que já foi calculado
                            base_total = total_m1_all
                            ...
                            if tipo_visualizacao == "CPU (Custo por Unidade)":
                                ...
                                bud = base_total / volume_real_m1 if volume_real_m1 != 0 else 0
                                flex_bud = flex_bud_total / volume_real_m2 if volume_real_m2 != 0 else 0
                                
                                # Flex Mês 1 - Mês 1 = Flex BUD - BUD (linha 1473)
                                flex_volume_delta = flex_bud - bud
                            else:
                                # Custo Total: Flex Mês 1 - Mês 1 = Flex BUD - BUD (linha 1473)
                                bud = base_total
                        ...
                        # remainder = Mês 2 - (Mês 1 + Flex Volume + sum(categorias))
                        remainder = round(total_m2_all - (total_m1_all + flex_volume_delta + sum(values_cats)), 2)
                        ...
                        values_waterfall = [total_m1_all]
                        measures_waterfall = ["absolute"]
                        
                        # Adicionar Flex Volume = Flex Mês 1 - Mês 1 sempre que o valor for diferente de zero
                        # O valor 364.36 K significa que o valor real é 364.36 (já dividido por 1000 se fator_conversao for K)
                        # IMPORTANTE: O fator de conversão já foi aplicado em total_m1_all e total_m2_all,
                        # então flex_volume_delta também já está com o fator aplicado
                        ...
                        if tem_flex_volume_significativo:
                            labels_waterfall.append("Flex Mês 1 - Mês 1")
                        ...
                        values_waterfall.append(total_m2_all)
                        ...
                        if "Outros" in labels_waterfall:
                            ...
                            cumulative_outros = total_m1_all
                        ...
                        with col3:
                            st.metric("Variação Total", f"{change_all:,.2f}", f"{(change_all/total_m1_all*100 if total_m1_all != 0 else 0):.1f}%")
                        with col4:
                        ...
                        if 'df_m1' in locals() and df_m1 is not None and len(df_m1) > 0 and 'df_m2' in locals() and df_m2 is not None and len(df_m2) > 0:
                            ...
                            if tem_custo_real:
                                try:
                                    ...
                                    # Obter volumes usando df_volume diretamente (mesma lógica do gráfico de volumes do app.py)
                                    # Aplicar os mesmos filtros que foram aplicados aos dados de custo
                                    volume_m1 = 0
                                    volume_m2 = 0
                                    
                                    if df_volume is not None and not df_volume.empty and 'Volume' in df_volume.columns:
                                        # Criar cópia do df_volume para aplicar filtros
                                        df_vol_filtrado = df_volume.copy()
                                        
                                        # Criar coluna Período_Ano no df_vol_filtrado se necessário (mesma lógica do df_filtrado_waterfall)
                                        # Inicializar col_mes_vol com valor padrão
                                        col_mes_vol = None
                                        
                                        if col_mes_waterfall == 'Período_Ano':
                                            if 'Período' in df_vol_filtrado.columns and 'Ano' in df_vol_filtrado.columns:
                                                df_vol_filtrado['Período_Ano'] = df_vol_filtrado['Período'].astype(str) + ' ' + df_vol_filtrado['Ano'].astype(str)
                                                col_mes_vol = 'Período_Ano'
                                            else:
                                                # Se não tiver Período e Ano, usar apenas Período
                                                col_mes_vol = 'Período' if 'Período' in df_vol_filtrado.columns else None
                                        elif col_mes_waterfall:
                                            col_mes_vol = col_mes_waterfall
                                        else:
                                            col_mes_vol = 'Período' if 'Período' in df_vol_filtrado.columns else None
                                        
                                        # Aplicar os mesmos filtros que foram aplicados aos dados de custo
                                        # (usar os mesmos filtros da sidebar que já foram aplicados em df_filtrado)
                                    ...
                                    df_reorganizado['% Mês 2/Flex Mês 1'] = (df_tabela_flex_waterfall['Total'] / df_tabela_flex_waterfall['Flex BUD'].replace(0, 1) * 100).fillna(0)
                                    ...
                                    if modo_tabela_flex_waterfall == "Fixo/Variável":
                                        ...
                                        # Modo "Total": agrupar sem separar por Custo
                                        ...
                                        if len(colunas_agrupamento_total) > 0:
                                            ...
                                            df_tabela_total_agrupado['% Mês 2/Flex Mês 1'] = df_tabela_total_agrupado.apply(
                                                lambda row: (row['Mês 2'] / row['Flex Mês 1'].replace(0, 1) * 100) if row['Flex Mês 1'] != 0 and pd.notnull(row['Flex Mês 1']) else 0,
                                                axis=1
                                            )
                                        else:
                                            df_tabela_total_agrupado = pd.DataFrame([{
                                                nome_mes_inicial_abrev: df_tabela_flex_waterfall[nome_mes_inicial_abrev].sum(),
                                                f'Flex {nome_mes_inicial_abrev.lower()} - {nome_mes_inicial_abrev.lower()}': df_tabela_flex_waterfall[f'Flex {nome_mes_inicial_abrev.lower()} - {nome_mes_inicial_abrev.lower()}'].sum(),
                                                f'Flex {nome_mes_inicial_abrev.lower()}': df_tabela_flex_waterfall[f'Flex {nome_mes_inicial_abrev.lower()}'].sum(),
                                                f'{nome_mes_final_abrev} - Flex {nome_mes_inicial_abrev.lower()}': df_tabela_flex_waterfall[f'{nome_mes_final_abrev} - Flex {nome_mes_inicial_abrev.lower()}'].sum(),
                                                f'{nome_mes_final_abrev.lower()}': df_tabela_flex_waterfall[f'{nome_mes_final_abrev.lower()}'].sum(),
                                                f'% {nome_mes_final_abrev.lower()}/flex {nome_mes_inicial_abrev.lower()}': (df_tabela_flex_waterfall[f'{nome_mes_final_abrev.lower()}'].sum() / df_tabela_flex_waterfall[f'Flex {nome_mes_inicial_abrev.lower()}'].sum() * 100) if df_tabela_flex_waterfall[f'Flex {nome_mes_inicial_abrev.lower()}'].sum() != 0 else 0
                                            }])
                                        
                                        # Exibir tabela total
                                        total_geral = df_tabela_total_agrupado[f'{nome_mes_final_abrev.lower()}'].sum()
                                        ...
                                        colunas_numericas = [col for col in df_tabela_total_agrupado.columns 
                                                            if col not in ['Type 05', 'Type 06', 'Account', 'Custo', 'Período']]
                                        df_display = df_tabela_total_agrupado[colunas_numericas].copy()
                                        ...
                                        # Calcular resumo usando os nomes corretos das colunas
                                        linha_resumo_total = {
                                            'Mês 1': df_tabela_total_agrupado['Mês 1'].sum(),
                                            'Flex Mês 1 - Mês 1': df_tabela_total_agrupado['Flex Mês 1 - Mês 1'].sum(),
                                            'Flex Mês 1': df_tabela_total_agrupado['Flex Mês 1'].sum(),
                                            'Mês 2 - Flex Mês 1': df_tabela_total_agrupado['Mês 2 - Flex Mês 1'].sum(),
                                            'Mês 2': df_tabela_total_agrupado['Mês 2'].sum(),
                                            '% Mês 2/Flex Mês 1': df_tabela_total_agrupado['% Mês 2/Flex Mês 1'].sum()
                                        }
                                        
                                        # Formatar resumo
                                        linha_resumo_total_formatado = {}
                                        for col in ['Mês 1', 'Flex Mês 1 - Mês 1', 'Flex Mês 1', 'Mês 2 - Flex Mês 1', 'Mês 2']:
                                            if tipo_visualizacao == "CPU (Custo por Unidade)":
                                                linha_resumo_total_formatado[col] = f"{linha_resumo_total[col]:,.2f}"
                                            else:
                                                sufixo = ""
                                                if fator_conversao:
                                                    if fator_conversao == "K (milhares)":
                                                        ...
                                                        sufixo = " M"
                                                linha_resumo_total_formatado[col] = f"{linha_resumo_total[col]:,.2f}{sufixo}"
                                        
                                        # Formatar percentual
                                        linha_resumo_total_formatado['% Mês 2/Flex Mês 1'] = f"{linha_resumo_total['% Mês 2/Flex Mês 1']:,.2f}%"
                                        ...
                                        if num_colunas > 0:
                                            cols = st.columns(num_colunas, gap="small")
                                            for idx, col_nome in enumerate(ordem_colunas_waterfall[:num_colunas]):
                                                if col_nome in linha_resumo_total_formatado:
                                                    with cols[idx]:
                                                        valor_formatado = linha_resumo_total_formatado.get(col_nome, '-')
                                                        st.markdown(f"<div style='font-size: 0.75rem;'><strong>{col_nome}</strong><br>{valor_formatado}</div>", unsafe_allow_html=True)
                                        
                                        html_table = criar_tabela_html_com_barra(df_display, linha_resumo_total_formatado)
                                        st.markdown(html_table, unsafe_allow_html=True)
            ...
            with tab_budget:
                ...
                st.info("ℹ️ Dados de budget não disponíveis. A tabela requer df_budget_filtrado, df_budget_vol_filtrado e df_volume_real_filtrado.")

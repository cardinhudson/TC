...
def encontrar_arquivo_parquet(nome_arquivo, ano_selecionado=None):
    ...
    1. Pasta Forecast (dados/TC_Ext/Forecast/) - PRIORIDADE MÁXIMA
    2. Se ano_selecionado for especificado: Pasta do ano (dados/TC_Ext/{ANO}/)
    3. Histórico consolidado (dados/TC_Ext/historico_consolidado/)
    4. Pasta do ano mais recente (dados/TC_Ext/{ANO}/)
    ...
    try:
        ...
        if arquivo_parquet is None:
            ...
            raise FileNotFoundError(
                "❌ Arquivo não encontrado: df_final.parquet\n"
                "💡 Verifique se o arquivo existe em:\n"
                "   - dados/TC_Ext/Forecast/forecast_completo.parquet (PRIORIDADE)\n"
...
_cfg_periodos = _carregar_config_forecast() or {}
...
with tab_visualizar:
    ...
    def _render_tab_visualizar():
        ...
        if not df_custos_especificos.empty:
            ...
            # Filtrar apenas linhas válidas (com Oficina, Período e Total)
            mask_valido = (
            ...
            if not df_custos_formatado.empty:
                ...
                def padronizar_colunas_custos(df, nome_tipo="Custos Específicos"):
                    ...
                    ordem_colunas_referencia = [
                        ...
                        'Total', 'Type 05', 'Type 06', 'USI', 'Valor', 'Veículo', 'Descricao'
                    ]
                ...
                if not HAS_AGGRID:
                    ...
                    selecionar_todos = st.checkbox("Selecionar Todos", value=False, key='sel_todos_fallback')
                    if selecionar_todos:
                        df_sel['✅'] = True
                    ...
                    with col_btn_edit:
                        if st.button("✏️ Editar Selecionada", use_container_width=True, key='btn_editar_fallback'):
                            if n_selecionadas == 1:
                                st.session_state['edit_custo_especifico_ext'] = linhas_marcadas.iloc[0].to_dict()
                                st.rerun()
                ...
                with col_btn_edit:
                    ...
                    if _sel_rows_edit is not None:
                        ...
                        _sel_rows_edit = []
                    if st.button("✏️ Editar Selecionada", use_container_width=True, key='btn_editar_aggrid'):
                        if len(_sel_rows_edit) == 1:
                            st.session_state['edit_custo_especifico_ext'] = _sel_rows_edit[0]
                            st.rerun()
                ...
                if 'edit_custo_especifico_ext' in st.session_state:
                    _ed = st.session_state['edit_custo_especifico_ext']
                    st.markdown("---")
                    st.markdown("#### ✏️ Editar Custo Selecionado")
                    with st.form("form_editar_custo_ext"):
                        col_e1, col_e2, col_e3, col_e4 = st.columns(4)
                        with col_e1:
                            st.text_input("Oficina", value=str(_ed.get('Oficina', '') or ''), disabled=True, key='edit_oficina_ext')
                        with col_e2:
                            st.text_input("Período", value=str(_ed.get('Período', '') or ''), disabled=True, key='edit_periodo_ext')
                        with col_e3:
                            st.text_input("Account", value=str(_ed.get('Account', '') or ''), disabled=True, key='edit_account_ext')
                        with col_e4:
                            _val_atual = pd.to_numeric(_ed.get('Total'), errors='coerce')
                            _val_atual = float(_val_atual) if pd.notna(_val_atual) else 0.0
                            novo_valor = st.number_input("Valor (R$)", value=_val_atual, format="%.2f", key='edit_valor_ext')
                        nova_descricao = st.text_input("Descrição", value=str(_ed.get('Descricao', '') or ''), key='edit_descricao_ext')
                        ...
                        if submitted:
                            ...
                            _old_val = pd.to_numeric(_ed.get('Total'), errors='coerce')
                            if pd.notna(_old_val) and 'Total' in df_custos_especificos.columns:
                                _mask_edit = _mask_edit & (pd.to_numeric(df_custos_especificos['Total'], errors='coerce') == _old_val)
                            _indices_edit = df_custos_especificos[_mask_edit].index.tolist()
                            if _indices_edit:
                                _idx = _indices_edit[0]
                                df_custos_especificos.at[_idx, 'Total'] = novo_valor
                                if 'Descricao' in df_custos_especificos.columns:
                                    df_custos_especificos.at[_idx, 'Descricao'] = nova_descricao
                                if salvar_custos_especificos(df_custos_especificos):
                                    del st.session_state['edit_custo_especifico_ext']
                ...
                st.download_button(
                    ...
                    key="btn_download_custos_completo_ext"
                )
            else:
...
if aplicar_config_forecast:
    ...
    def calcular_medias_forecast(df_filtrado_cache, colunas_adicionais_cache, periodos_para_media_cache, ultimo_periodo_dados_cache=None):
        ...
        if periodos_para_media_cache and 'Período' in df_filtrado_cache.columns:
            ...
            # NOTA: debug_calcular_medias removido do cache (st.session_state proibido em @st.cache_data)
    ...
    with st.spinner("🔄 Gerando arquivos de forecast... Isso pode levar alguns minutos."):
        try:
            ...
            pasta_dados = "dados"
            pasta_forecast = os.path.join(pasta_dados, "TC_Ext", "Forecast")
            try:
                # Criar pasta dados se não existir
                if not os.path.exists(pasta_dados):
                    os.makedirs(pasta_dados, exist_ok=True)
                
                # Criar pasta Forecast dentro de dados/TC_Ext
            ...
            try:
                if oficinas_ref_budget_set:
                    ...
                    if os.path.exists(caminho_budget_custo) and os.path.exists(caminho_budget_vol):
                        ...
                        adicionar_mensagem('warning', '⚠️ Budget ref: arquivos BUD não encontrados em dados/TC_Ext/historico_consolidado/BUD')
            except Exception:
                pass
                    ...
                    if not vol_mes_df.empty:
                        ...
                        if usa_veiculo_no_mes:
                            ...
                            for idx in df_forecast_completo.index:
                                chave = (str(df_forecast_completo.at[idx, 'Oficina']), str(df_forecast_completo.at[idx, 'Veículo']))
                            ...
                            volume_medio_count = 0
                            for idx in df_forecast_completo.index:
                                chave = (str(df_forecast_completo.at[idx, 'Oficina']),)
                                if chave in volume_dict:
                                    volume_valores.append(volume_dict[chave])
                                    volume_encontrado_count += 1
                                elif 'Volume_Medio_Historico' in df_forecast_completo.columns:
            ...
            # Criar pasta Forecast em dados/TC_Ext/Forecast (ANTES de tentar salvar) (MESMA LÓGICA DO FORECAST COPY)
            pasta_dados = "dados"
            pasta_forecast = os.path.join(pasta_dados, "TC_Ext", "Forecast")

...
class TestRankingConsolidado:
    ...
    def test_inclui_primeiro_ganho_apos_perdas(self):
        merged = pd.DataFrame(
            [
                {"Type 06": "Maintenance", "Desvio": 500.0},
                {"Type 06": "Material Losses", "Desvio": 180.0},
                {"Type 06": "Energy", "Desvio": -2.0},
                {"Type 06": "Trips", "Desvio": -20.0},
            ]
        )
        result = _selecionar_type06_para_alerta(merged, top_n=10)
        desvios = result["Desvio"].tolist()
        assert desvios == [500.0, 180.0, -2.0]

    def test_ordem_type06_segue_delta_das_accounts(self):
        merged = pd.DataFrame(
            [
                {"Type 06": "Maintenance", "Desvio": 501.7},
                {"Type 06": "Material Losses", "Desvio": -320.0},
                {"Type 06": "Energy", "Desvio": -11.3},
                {"Type 06": "Consumption Material", "Desvio": -2.9},
            ]
        )
        merged_acc = pd.DataFrame(
            [
                {"Type 06": "Maintenance", "Account": "Material", "Desvio_Acc": 657.5},
                {"Type 06": "Material Losses", "Account": "Scrap Sales", "Desvio_Acc": 183.1},
                {"Type 06": "Energy", "Account": "Energy Long Term lease", "Desvio_Acc": -1.8},
                {"Type 06": "Consumption Material", "Account": "Consumption Material", "Desvio_Acc": -2.9},
                {"Type 06": "Trips", "Account": "Trips", "Desvio_Acc": 0.0},
            ]
        )

        result = _selecionar_type06_para_alerta(
            merged,
            top_n=10,
            merged_acc=merged_acc,
        )
        assert result["Type 06"].tolist() == [
            "Maintenance",
            "Material Losses",
            "Energy",
        ]
...
class TestTeamsCardConsolidado:
    ...
    def test_card_com_tabela(self):
        ...
        assert "Tabela de Validação (Top 10)" in text
        assert "Realizado" in text

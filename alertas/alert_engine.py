...
def _selecionar_type06_para_alerta(
    merged: pd.DataFrame,
    top_n: int,
    merged_acc: pd.DataFrame | None = None,
) -> pd.DataFrame:
    """Seleciona perdas e inclui o primeiro ganho mais proximo de zero.

    Regras:
    - Prioriza a mesma ordem da tabela de validacao (delta por Account).
    - Deduplica por Type 06.
    - Ignora linhas nulas (abs(delta) <= 0.01).
    - Mantem os maiores desvios positivos ate `top_n`.
    - Acrescenta o primeiro item com ganho/empate (desvio <= 0),
      para expor o ponto de virada no alerta.
    """
    if merged_acc is not None and not merged_acc.empty:
        acc = merged_acc.copy()
        acc = acc[acc["Desvio_Acc"].abs() > 0.01]
        acc = acc.sort_values("Desvio_Acc", ascending=False)

        ordered_type06: list[str] = []
        positivos_sel = 0

        for _, row in acc.iterrows():
            t6 = str(row["Type 06"])
            desvio_acc = float(row["Desvio_Acc"])
            if t6 in ordered_type06:
                continue

            if desvio_acc > 0:
                if positivos_sel < top_n:
                    ordered_type06.append(t6)
                    positivos_sel += 1
                continue
        ...
        if ordered_type06:
            base = merged.set_index("Type 06", drop=False)
            ordered_validos = [t6 for t6 in ordered_type06 if t6 in base.index]
            if ordered_validos:
                return base.loc[ordered_validos].reset_index(drop=True)

    ordenado = merged.sort_values("Desvio", ascending=False).copy()
    perdas = ordenado[ordenado["Desvio"] > 0].head(top_n)
    ganhos = ordenado[ordenado["Desvio"] <= 0].head(1)

    selecionados = pd.concat([perdas, ganhos], ignore_index=True)
    selecionados = selecionados.drop_duplicates(subset=["Type 06"])
    return selecionados
...
def calcular_ranking_por_oficina(
    ...
    for ofi in oficinas_iter:
        ...
        acc_merged_all = real_acc.merge(
            esp_acc[["Type 06", "Account", "Esperado_Acc"]],
            on=["Type 06", "Account"],
            how="outer",
        )
        acc_merged_all["Real_Acc"] = acc_merged_all["Real_Acc"].fillna(0)
        acc_merged_all["Esperado_Acc"] = acc_merged_all["Esperado_Acc"].fillna(0)
        acc_merged_all["Desvio_Acc"] = (
            acc_merged_all["Real_Acc"] - acc_merged_all["Esperado_Acc"]
        )

        perdas = _selecionar_type06_para_alerta(
            merged,
            top_n,
            acc_merged_all,
        )

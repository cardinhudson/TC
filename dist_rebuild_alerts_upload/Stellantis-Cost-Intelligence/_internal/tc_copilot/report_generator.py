...
def _formatar_tempo_decorrido(
    segundos_totais: float | None,
    idioma: str,
) -> str | None:
    """Formata o tempo decorrido para exibição no PDF."""
    if segundos_totais is None or segundos_totais <= 0:
        return None
    ...
    lbl_t = "Tempo de geração" if idioma == "pt-BR" else "Generation time"
    return f"{lbl_t}: {tempo_str}"


def _executar_build_pdf(
    pdf_path: str,
    tempo_inicio: float | None,
    build_fn: Callable[[str, float | None], None],
    descricao: str,
) -> None:
    """Gera o PDF final com tempo mais fiel usando duas passagens."""
    if tempo_inicio is None:
        build_fn(pdf_path, None)
        return

    pdf_tmp = f"{pdf_path}.timing.tmp.pdf"
    try:
        t_build_1 = _time.time()
        build_fn(pdf_tmp, max(_time.time() - tempo_inicio, 0.0))
        duracao_build_1 = max(_time.time() - t_build_1, 0.0)

        tempo_final_estimado = max(
            (_time.time() - tempo_inicio) + duracao_build_1,
            0.0,
        )
        logger.info(
            "[%s] build1=%.1f seg | tempo_exibido=%.1f seg",
            descricao,
            duracao_build_1,
            tempo_final_estimado,
        )

        build_fn(pdf_path, tempo_final_estimado)
    finally:
        try:
            if os.path.exists(pdf_tmp):
                os.remove(pdf_tmp)
        except Exception:
            logger.warning("Nao foi possivel remover arquivo temporario: %s", pdf_tmp)
...
def _construir_capa(
    ...
    # ── Rodapé da capa: gerado por IA + data + tempo ────
    elements.append(Spacer(1, 0.8 * cm))
    texto_rodape = (
        "Relatório gerado por algoritmo de Inteligência Artificial — SCI TC Copilot"
        if idioma == "pt-BR"
        else "Report generated by AI algorithm — SCI TC Copilot"
    )
    elements.append(Paragraph(texto_rodape, estilos["rodape_capa"]))

    agora_rodape = datetime.now()
    data_geracao = agora_rodape.strftime("%d/%m/%Y às %H:%M")
    lbl_data = f"Data de geração: {data_geracao}" if idioma == "pt-BR" else f"Generated on: {data_geracao}"
    elements.append(Paragraph(lbl_data, estilos["rodape_capa"]))

    tempo_formatado = _formatar_tempo_decorrido(
        tempo_total_segundos,
        idioma,
    )
    if tempo_formatado:
        elements.append(Paragraph(tempo_formatado, estilos["rodape_capa"]))
...
def gerar_pdf(ano: int, idioma: str = "pt-BR", simbolo_moeda: str = "€", modo: str = "ia", tempo_inicio: float | None = None) -> str:
    """
    ...
    def _build_pdf_once(path_saida: str, tempo_total_segundos: float | None):
...
_TABELA_COLS_GLOBAL = [
    ...
    "Oficina", "Texto breve", "Fornecedor",
]
_TABELA_COLS_OFC = [
    "Type 05", "Type 06", "Account",
...
def gerar_relatorio_mes(
    ...
    """
    _t0 = tempo_inicio if tempo_inicio is not None else _time.time()
    ...
    try:
        ...
        pdf_mensal = gerar_pdf_mensal(ano, mes_numero, modo="ia", idioma=idioma, simbolo_moeda=simbolo, tempo_inicio=_t0)
        if pdf_mensal:
    ...
    pdf_anual = gerar_pdf(ano, idioma, simbolo_moeda=simbolo, modo="ia", tempo_inicio=_t0)
...
def gerar_pdf_local(ano: int, idioma: str = "pt-BR", simbolo_moeda: str = "R$", tempo_inicio: float | None = None) -> str:
    """
...
def gerar_pdf_mensal(
    ...
    def _build_pdf_once(path_saida: str, tempo_total_segundos: float | None):
        ...
        lbl_data = (
            ...
            if idioma == "pt-BR"
            else f"Generated on: {data_geracao}"
        )
        elements.append(Paragraph(lbl_data, estilos["rodape_capa"]))
    ...
    try:
        _executar_build_pdf(
            ...
            tempo_inicio,
            _build_pdf_once,
            f"PDF mensal {mes_numero:02d}/{ano} ({modo})",
        )
...
def gerar_relatorio_mes_local(
    ...
    try:
        from tc_copilot.relatorios_db import registrar_pdf
        pdf_mensal = gerar_pdf_mensal(ano, mes_numero, modo="local", idioma=idioma, simbolo_moeda=simbolo, tempo_inicio=_t0)
        if pdf_mensal:
    ...
    pdf_anual = gerar_pdf_local(ano, idioma, simbolo_moeda=simbolo, tempo_inicio=_t0)

...
def obter_data_atualizacao_dados():
    ...
    try:
        ...
        arquivos_dados = [
            ...
            os.path.join("dados", "TC_Principal", "historico_consolidado", "BUD", "df_principal_historico_BUD.parquet"),
            # Caminhos do Best Estimate (arquivos gerados)
            os.path.join("dados", "TC_Principal", "Forecast", "forecast_completo.parquet"),
            os.path.join("dados", "TC_Principal", "Forecast", "df_vol_historico.parquet"),
            ...
            try:
                ...
                if anos:
                    ...
                    arquivos_dados.extend([
                        ...
                        os.path.join(pasta_dados, ano_mais_recente, "df_vol_veiculos.parquet"),
                    ])
...
st.markdown("""
    <style>
        ...
        button[key="btn_brl_hidden_tc"],
        button[key="btn_usd_hidden_tc"],
        button[key="btn_eur_hidden_tc"] {
        ...
        div:has(button[key="btn_brl_hidden_tc"]),
        div:has(button[key="btn_usd_hidden_tc"]),
        div:has(button[key="btn_eur_hidden_tc"]) {
            display: none !important;
            visibility: hidden !important;
            height: 0 !important;
            width: 0 !important;
        ...
        div[data-testid="stButton"]:has(button[key="btn_brl_hidden_tc"]),
        div[data-testid="stButton"]:has(button[key="btn_usd_hidden_tc"]),
        div[data-testid="stButton"]:has(button[key="btn_eur_hidden_tc"]) {
            display: none !important;
            height: 0 !important;
            margin: 0 !important;
            padding: 0 !important;
        ...
        *:not(script):not(style):not(#flags-container-top) {{
        ...
        div[data-testid="stNumberInput"][key="taxa_usd_para_brl_input_tc"] label,
        div[data-testid="stNumberInput"][key="taxa_eur_para_brl_input_tc"] label {
            font-size: 0.7rem !important;
            white-space: nowrap !important;
            word-break: keep-all !important;
            overflow: visible !important;
        ...
        div[data-testid="stNumberInput"][key="taxa_usd_para_brl_input_tc"] label p,
        div[data-testid="stNumberInput"][key="taxa_eur_para_brl_input_tc"] label p {
            font-size: 0.7rem !important;
            white-space: nowrap !important;
            word-break: keep-all !important;
            overflow: visible !important;
        ...
        div[data-testid="stNumberInput"][key="taxa_usd_para_brl_input_tc"],
        div[data-testid="stNumberInput"][key="taxa_eur_para_brl_input_tc"] {
            overflow: visible !important;
        }
        div[data-testid="stNumberInput"][key="taxa_usd_para_brl_input_tc"] > div,
        div[data-testid="stNumberInput"][key="taxa_eur_para_brl_input_tc"] > div {
            overflow: visible !important;
        }
        /* Forçar que o label completo seja visível */
        div[data-testid="stNumberInput"][key="taxa_usd_para_brl_input_tc"] > div > label,
        div[data-testid="stNumberInput"][key="taxa_eur_para_brl_input_tc"] > div > label {
            overflow: visible !important;
            width: auto !important;
            max-width: none !important;
        }
        ...
        div[data-testid="stNumberInput"][key="taxa_usd_para_brl_input_tc"] label p:contains("R$"),
        div[data-testid="stNumberInput"][key="taxa_eur_para_brl_input_tc"] label p:contains("R$") {
            white-space: nowrap !important;
            word-break: keep-all !important;
            overflow: visible !important;
            text-overflow: clip !important;
        ...
        div[data-testid="stRadio"][key="tipo_visualizacao_top_tc"] label,
        div[data-testid="stRadio"][key="fator_conversao_top_tc"] label {
            font-size: 0.7rem !important;
        }
        div[data-testid="stRadio"][key="tipo_visualizacao_top_tc"] label p,
        div[data-testid="stRadio"][key="fator_conversao_top_tc"] label p {
            font-size: 0.7rem !important;
            line-height: 1.2 !important;
        }
""", unsafe_allow_html=True)
...
def listar_anos_disponiveis():
    ...
    return _core_listar_anos_disponiveis(os.path.join("dados", "TC_Principal"))
...
if 'current_saved_theme_tc' not in st.session_state:
    st.session_state.current_saved_theme_tc = get_current_theme()

# Mostrar tema atual
st.sidebar.caption(f"Tema ativo: **{st.session_state.current_saved_theme_tc.upper()}**")

# Criar duas colunas para os botões
col_dark, col_light = st.sidebar.columns(2)

# Inicializar flag de mensagem
if 'show_reload_message_tc' not in st.session_state:
    st.session_state.show_reload_message_tc = False
if 'theme_needs_reload_tc' not in st.session_state:
    st.session_state.theme_needs_reload_tc = False

# Botão Dark Mode (Lua)
with col_dark:
    if st.button("🌙 Dark", key="btn_dark_tc", help="Ativar Dark Mode", width="stretch"):
        if save_theme_to_config('dark'):
            st.session_state.current_saved_theme_tc = 'dark'
            st.session_state.show_reload_message_tc = True
            st.session_state.theme_needs_reload_tc = True

# Botão Light Mode (Sol)
with col_light:
    if st.button("☀️ Light", key="btn_light_tc", help="Ativar Light Mode", width="stretch"):
        if save_theme_to_config('light'):
            st.session_state.current_saved_theme_tc = 'light'
            st.session_state.show_reload_message_tc = True
            st.session_state.theme_needs_reload_tc = True

# Mostrar mensagem se tema foi alterado
if st.session_state.show_reload_message_tc:
    st.sidebar.success(f"✅ Tema **{st.session_state.current_saved_theme_tc.upper()}** salvo!")
    st.sidebar.info("🔄 Aplicando tema...")
    if st.session_state.theme_needs_reload_tc:
        st.session_state.theme_needs_reload_tc = False
        st.session_state.show_reload_message_tc = False
        components.html(
            """
...
if 'filtro_ano_tc_veic' not in st.session_state:
    st.session_state.filtro_ano_tc_veic = opcoes_ano[index_padrao] if index_padrao < len(opcoes_ano) else "Todos"

# Seletor de ano
ano_selecionado = st.sidebar.selectbox(
    "Selecione o ano:",
    options=opcoes_ano,
    index=opcoes_ano.index(st.session_state.filtro_ano_tc_veic) if st.session_state.filtro_ano_tc_veic in opcoes_ano else index_padrao,
    help="Selecione 'Todos' para ver dados consolidados ou um ano específico",
    key="filtro_ano_tc_veic_selectbox"
)
...
if is_main_page:
    ...
    with col_taxa1:
        ...
        taxa_usd_para_brl = st.number_input(
            ...
            key="taxa_usd_para_brl_input_tc",
            label_visibility="collapsed"
        )
    ...
    with col_taxa2:
        ...
        taxa_eur_para_brl = st.number_input(
            ...
            key="taxa_eur_para_brl_input_tc",
            label_visibility="collapsed"
        )

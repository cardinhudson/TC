...
def encontrar_arquivo_parquet(nome_arquivo, ano_selecionado=None):
    ...
    1. Pasta Forecast (dados/TC_Principal/Forecast/) - PRIORIDADE MÁXIMA
    2. Se ano_selecionado for especificado: Pasta do ano (dados/TC_Principal/{ANO}/)
    3. Histórico consolidado (dados/TC_Principal/historico_consolidado/)
    4. Pasta do ano mais recente (dados/TC_Principal/{ANO}/)
    ...
    if nome_arquivo == "df_vol_veiculos.parquet":
        caminho_forecast_vol = os.path.join("dados", "TC_Principal", "Forecast", "df_vol_historico.parquet")
    ...
    if ano_selecionado is not None and ano_selecionado != "Todos":
        caminho_ano = os.path.join("dados", "TC_Principal", str(ano_selecionado), nome_arquivo)
        if os.path.exists(caminho_ano):
            return caminho_ano
    
    # 2. Tentar histórico consolidado (fallback)
    caminho_historico = os.path.join("dados", "TC_Principal", "historico_consolidado", nome_arquivo.replace(".parquet", "_historico.parquet"))
    if os.path.exists(caminho_historico):
        return caminho_historico
    
    # TC Veículos: volume historico uses different naming
    if "vol" in nome_arquivo:
        caminho_hist_vol = os.path.join("dados", "TC_Principal", "historico_consolidado", "df_vol_historico.parquet")
...
def load_data(ano_selecionado_param):
    ...
    try:
        ...
        arquivo_parquet = encontrar_arquivo_parquet("df_principal.parquet", ano_para_busca)

        if arquivo_parquet is None:
            st.error(f"❌ Arquivo não encontrado: df_principal.parquet")
            st.info("💡 Verifique se o arquivo existe em:")
            st.info("   - dados/TC_Principal/Forecast/forecast_completo.parquet (PRIORIDADE)")
            st.info("   - dados/TC_Principal/historico_consolidado/df_principal_historico.parquet")
            st.info("   - dados/TC_Principal/{ANO}/df_principal.parquet")
            st.stop()
...
def load_volume_data(ano_selecionado_param):
    ...
    try:
        ...
        ano_para_busca = None if ano_selecionado_param == "Todos" else ano_selecionado_param
        arquivo_parquet = encontrar_arquivo_parquet("df_vol_veiculos.parquet", ano_para_busca)
...
def load_volume_historico_data():
    ...
    try:
        ...
        caminho_forecast = os.path.join("dados", "TC_Principal", "Forecast", "df_vol_historico.parquet")
        ...
        caminho_historico = os.path.join("dados", "TC_Principal", "historico_consolidado", "df_vol_historico.parquet")
        if os.path.exists(caminho_historico):
...
def get_veiculos_budget_opcoes(ano_selecionado_param):
    ...
    try:
        ...
        if os.path.exists(caminho_budget_custo):
            try:
                df_budget = pd.read_parquet(caminho_budget_custo, columns=["Veículo", "Ano"])
            except Exception:
                df_budget = pd.read_parquet(caminho_budget_custo)

            if df_budget is not None and not df_budget.empty and 'Veículo' in df_budget.columns:
                if (
                    ano_selecionado_param is not None
                    and ano_selecionado_param != "Todos"
                    and 'Ano' in df_budget.columns
                    ...
                    try:
                        ...
                        pass
                veiculos_set.update(df_budget['Veículo'].dropna().astype(str).unique().tolist())
        ...
        if os.path.exists(caminho_budget_vol):
            ...
            if df_vol is not None and not df_vol.empty and 'Veículo' in df_vol.columns:
                if (
                    ano_selecionado_param is not None
                    and ano_selecionado_param != "Todos"
                    and 'Ano' in df_vol.columns
                    ...
                    try:
                        ...
                        pass
                veiculos_set.update(df_vol['Veículo'].dropna().astype(str).unique().tolist())
        return sorted(veiculos_set)
    except Exception:
        return []
...
if st.sidebar.button("🔄 Atualizar Dados", use_container_width=True):
    ...
    if 'debug_calcular_medias_tc' in st.session_state:
        del st.session_state['debug_calcular_medias_tc']
...
if 'filtro_oficina_simulador_tc' not in st.session_state:
    st.session_state.filtro_oficina_simulador_tc = ["Todos"]
if 'filtro_veiculo_simulador_tc' not in st.session_state:
    st.session_state.filtro_veiculo_simulador_tc = ["Todos"]
if 'filtro_usi_simulador_tc' not in st.session_state:
    st.session_state.filtro_usi_simulador_tc = ["TC Ext"]
if 'filtro_periodo_simulador_tc' not in st.session_state:
    st.session_state.filtro_periodo_simulador_tc = ["Todos"]
...
if df_total is not None and 'Oficina' in df_total.columns:
    ...
    default_oficina = st.session_state.filtro_oficina_simulador_tc if all(x in oficina_opcoes for x in st.session_state.filtro_oficina_simulador_tc) else ["Todos"]
    oficina_selecionadas = st.sidebar.multiselect(
        "Selecione a Oficina:", oficina_opcoes, default=default_oficina, key="filtro_oficina_simulador_multiselect_tc"
    )
    st.session_state.filtro_oficina_simulador_tc = oficina_selecionadas if oficina_selecionadas else ["Todos"]
...
if df_total is not None and 'Veículo' in df_total.columns:
    ...
    default_veiculo = st.session_state.filtro_veiculo_simulador_tc if all(x in veiculo_opcoes for x in st.session_state.filtro_veiculo_simulador_tc) else ["Todos"]
    veiculo_selecionados = st.sidebar.multiselect(
        "Selecione o Veículo:", veiculo_opcoes, default=default_veiculo, key="filtro_veiculo_simulador_multiselect_tc"
    )
    st.session_state.filtro_veiculo_simulador_tc = veiculo_selecionados if veiculo_selecionados else ["Todos"]

# Filtro 3: USI
usi_selecionada = ["TC Ext"]
if df_total is not None and 'USI' in df_total.columns:
    usi_opcoes = get_filter_options(df_total, 'USI')
    default_usi = st.session_state.filtro_usi_simulador_tc if all(x in usi_opcoes for x in st.session_state.filtro_usi_simulador_tc) else (["TC Ext"] if "TC Ext" in usi_opcoes else ["Todos"])
    usi_selecionada = st.sidebar.multiselect(
        "Selecione a USI:", usi_opcoes, default=default_usi, key="filtro_usi_simulador_multiselect_tc"
    )
    st.session_state.filtro_usi_simulador_tc = usi_selecionada if usi_selecionada else ["TC Ext"]
...
if df_total is not None and 'Período' in df_total.columns:
    ...
    default_periodo = st.session_state.filtro_periodo_simulador_tc if all(x in periodo_opcoes for x in st.session_state.filtro_periodo_simulador_tc) else ["Todos"]
    periodo_selecionado = st.sidebar.multiselect(
        "Selecione o Período:", periodo_opcoes, default=default_periodo, key="filtro_periodo_simulador_multiselect_tc"
    )
    st.session_state.filtro_periodo_simulador_tc = periodo_selecionado if periodo_selecionado else ["Todos"]
...
if df_total is not None:
    # 🔧 CORREÇÃO CRÍTICA: Criar df_para_grafico_periodo ANTES de aplicar o filtro de Período (mesma lógica do TC_Principal linha 350)
...
# ====================================================================
# 🔧 MODO CPU: Preparar dados para visualização (mesma lógica do TC_Principal)
# ====================================================================
...
if df_filtrado is not None and tipo_visualizacao == "CPU (Custo por Unidade)":
    ...
    if df_vol_calc is not None and 'Volume' in df_vol_calc.columns:
        ...
        if ('Oficina' in df_filtrado.columns and
            ...
            if 'Custo FP' in df_filtrado.columns:
                ...
                st.warning(
                    "⚠️ Colunas 'Custo FP' ou 'Despesa Primaria' necessárias para "
                    "calcular CPU"
                )
                df_visualizacao = df_filtrado.copy()
                coluna_visualizacao = (
                    'Custo FP' if 'Custo FP' in df_filtrado.columns else 'Despesa Primaria'
                )
    ...
    if 'Custo FP' in df_filtrado.columns:
        ...
        df_visualizacao = df_filtrado.copy()
        coluna_visualizacao = 'Despesa Primaria'
    else:
        df_visualizacao = df_filtrado.copy()
        coluna_visualizacao = 'Custo FP'

# 🔧 CORREÇÃO CRÍTICA: Adicionar Volume ao df_visualizacao mesmo quando não está no modo CPU
# Isso garante que o gráfico de Volume por Veículo use os mesmos dados do TC_Principal
...
if 'Veículo' in df_visualizacao.columns and 'Período' in df_visualizacao.columns:
    ...
    if df_vol_calc is not None and 'Volume' in df_vol_calc.columns:
        ...
        if tem_veiculo and 'Veículo' in df_vol_calc.columns:
            ...
            # 🔧 CORREÇÃO: Seguir a mesma lógica do TC_Principal - não normalizar Período antes do merge
            ...
            # 🔧 CORREÇÃO CRÍTICA: Sempre somar 'Custo FP' se existir, não apenas coluna_visualizacao
            # Isso garante que quando há múltiplas linhas com mesma combinação, o Total seja somado corretamente
            colunas_para_somar = []
            if 'Custo FP' in df_visualizacao.columns:
                colunas_para_somar.append('Custo FP')
            if coluna_visualizacao in df_visualizacao.columns and coluna_visualizacao != 'Custo FP':
...
if tipo_visualizacao == "CPU (Custo por Unidade)":
    ...
    if 'Custo FP' in df_visualizacao.columns and 'Volume' in df_visualizacao.columns:
        total_sum = pd.to_numeric(df_visualizacao['Custo FP'], errors='coerce').fillna(0).sum()
    ...
    if 'Despesa Primaria' in df_visualizacao.columns:
        # Converter para numérico caso seja categórico
        valor_series = pd.to_numeric(df_visualizacao['Despesa Primaria'], errors='coerce')
    ...
    if 'Custo FP' in df_visualizacao.columns:
        # Converter para numérico caso seja categórico
        total_series = pd.to_numeric(df_visualizacao['Custo FP'], errors='coerce')
...
caminho_historico = os.path.join("dados", "TC_Principal", "historico_consolidado", "df_principal_historico.parquet")
...
if 'Type 06' in df_filtrado.columns:
    ...
    if len(type06_valores) > 0:
        ...
        st.session_state.modo_config_sensibilidade_tc = 'global' if modo_config == "🌐 Global (Fixo/Variável)" else 'detalhado'
        
        if modo_config == "🌐 Global (Fixo/Variável)":
            # Modo global (original)
            
            # Inicializar session_state para modo global
            if 'sensibilidade_fixo_aplicada_tc' not in st.session_state:
                st.session_state.sensibilidade_fixo_aplicada_tc = 0.0
            if 'sensibilidade_variavel_aplicada_tc' not in st.session_state:
                st.session_state.sensibilidade_variavel_aplicada_tc = 1.0
            if 'inflacao_global_aplicada_tc' not in st.session_state:
                st.session_state.inflacao_global_aplicada_tc = 0.0
            ...
            with col_sens1:
                sensibilidade_fixo_temp = st.slider(
                    ...
                    min_value=0.0,
                    max_value=1.0,
                    value=st.session_state.sensibilidade_fixo_aplicada_tc,
                    step=0.05,
            ...
            with col_sens2:
                sensibilidade_variavel_temp = st.slider(
                    ...
                    min_value=0.0,
                    max_value=1.0,
                    value=st.session_state.sensibilidade_variavel_aplicada_tc,
                    step=0.05,
            ...
            with col_infl:
                ...
                inflacao_global_temp = st.slider(
                    ...
                    min_value=0.0,
                    max_value=20.0,
                    value=st.session_state.inflacao_global_aplicada_tc,
                    step=0.5,
                    format="%.2f",
            ...
            if 'sensibilidade_fixo_aplicada_tc' in st.session_state:
                sensibilidade_fixo = st.session_state.sensibilidade_fixo_aplicada_tc
            else:
                sensibilidade_fixo = sensibilidade_fixo_temp
            
            if 'sensibilidade_variavel_aplicada_tc' in st.session_state:
                sensibilidade_variavel = st.session_state.sensibilidade_variavel_aplicada_tc
            else:
                sensibilidade_variavel = sensibilidade_variavel_temp
            
            if 'inflacao_global_aplicada_tc' in st.session_state:
                inflacao_global = st.session_state.inflacao_global_aplicada_tc
            else:
            ...
            if 'valores_temp_sens_tc' not in st.session_state:
                st.session_state.valores_temp_sens_tc = {}
            if 'valores_temp_infl_tc' not in st.session_state:
                st.session_state.valores_temp_infl_tc = {}
            if 'widget_key_counter_tc' not in st.session_state:
                st.session_state.widget_key_counter_tc = 0
            ...
            with col_btn1:
                if st.button("🔵 Todos Fixos (0.0)", use_container_width=True, key="btn_fixos_tc"):
                    for type06 in type06_valores:
                        st.session_state.valores_temp_sens_tc[type06] = 0.0
                    st.rerun()
            
            with col_btn2:
                if st.button("🟠 Todos Variáveis (1.0)", use_container_width=True, key="btn_variaveis_tc"):
                    for type06 in type06_valores:
                        st.session_state.valores_temp_sens_tc[type06] = 1.0
                    st.rerun()
            
            with col_btn3:
                if st.button("⚖️ Todos Médios (0.5)", use_container_width=True, key="btn_medios_tc"):
                    for type06 in type06_valores:
                        st.session_state.valores_temp_sens_tc[type06] = 0.5
                    st.rerun()
            
            with col_btn4:
                if st.button("🧹 Limpar Configurações", use_container_width=True, key="btn_limpar_tc"):
                    st.session_state.sensibilidades_aplicadas_tc = None
                    st.session_state.inflacao_aplicada_tc = None
                    st.session_state.valores_temp_sens_tc = {}
                    st.session_state.valores_temp_infl_tc = {}
            ...
            with col_infl_input:
                inflacao_rapida = st.number_input(
                    ...
                    step=0.5,
                    format="%.2f",
                    key="inflacao_rapida_input_tc",
                    help="Digite o valor e clique em 'Aplicar a Todas'"
                )
            
            with col_infl_btn1:
                st.markdown("<br>", unsafe_allow_html=True)  # Espaçamento
                if st.button("📈 Aplicar a Todas", use_container_width=True, key="btn_aplicar_inflacao_tc"):
                    # Aplicar o novo valor a todas as linhas
                    for type06 in type06_valores:
                        st.session_state.valores_temp_infl_tc[type06] = inflacao_rapida
                    # Incrementar contador para forçar recriação dos widgets
                    st.session_state.widget_key_counter_tc += 1
                    st.rerun()
            
            with col_infl_btn2:
                st.markdown("<br>", unsafe_allow_html=True)  # Espaçamento
                if st.button("🔄 Zerar Inflação", use_container_width=True, key="btn_zerar_inflacao_tc"):
                    # Aplicar zero a todas as linhas
                    for type06 in type06_valores:
                        st.session_state.valores_temp_infl_tc[type06] = 0.0
                    # Incrementar contador para forçar recriação dos widgets
                    st.session_state.widget_key_counter_tc += 1
                    st.rerun()
            ...
            for type06 in type06_valores:
                ...
                if type06 in st.session_state.valores_temp_sens_tc:
                    valor_padrao_sens = st.session_state.valores_temp_sens_tc[type06]
                
                valor_padrao_infl = 0.0
                if type06 in st.session_state.valores_temp_infl_tc:
                    valor_padrao_infl = st.session_state.valores_temp_infl_tc[type06]
                ...
                with col4:
                    ...
                    st.session_state.valores_temp_sens_tc[type06] = sens
                ...
                with col6:
                    ...
                    widget_key = f"infl_{type06}_{st.session_state.widget_key_counter_tc}"
                    ...
                    # Atualizar valor temporário
                    st.session_state.valores_temp_infl_tc[type06] = inflacao
            
            # Botão para aplicar configurações
            st.markdown("---")
            
            # Armazenar configurações temporárias em session_state
            if 'sensibilidades_aplicadas_tc' not in st.session_state:
                st.session_state.sensibilidades_aplicadas_tc = None
            if 'inflacao_aplicada_tc' not in st.session_state:
                st.session_state.inflacao_aplicada_tc = None
            ...
            if st.session_state.sensibilidades_aplicadas_tc is not None:
                sensibilidades_type06 = st.session_state.sensibilidades_aplicadas_tc
                inflacao_type06 = st.session_state.inflacao_aplicada_tc
            else:
...
def carregar_custos_especificos():
    ...
    caminho_custos = os.path.join("dados", "TC_Principal", "Forecast", "custos_especificos.parquet")
...
def salvar_custos_especificos(df):
    ...
    pasta_forecast = os.path.join("dados", "TC_Principal", "Forecast")
...
colunas_tabela_custos = [
    ...
    'Custo FP', 'Despesa Primaria', 'Centocst', 'Fornec.', 'Fornecedor', 'USI',
        ...
        if not df_custos_formatado.empty:
            ...
            def padronizar_colunas_custos(df, nome_tipo="Custos Específicos"):
                ...
                ordem_colunas_referencia = [
                    ...
                    'Custo FP', 'Type 05', 'Type 06', 'USI', 'Despesa Primaria', 'Veículo'
                ]
            ...
            if not HAS_AGGRID:
                ...
                indices_selecionados = st.multiselect(
                    ...
                    key='fallback_indices_deletar_tc',
                )
...
with tab_adicionar:
    ...
    with col1:
        ...
        if 'account_info_cache_tc' not in st.session_state:
            st.session_state.account_info_cache_tc = {}
        
        # Inicializar session_state para os valores dos campos
        if 'type06_valor_tc' not in st.session_state:
            st.session_state.type06_valor_tc = ""
        if 'type05_valor_tc' not in st.session_state:
            st.session_state.type05_valor_tc = ""
        if 'custo_valor_tc' not in st.session_state:
            st.session_state.custo_valor_tc = ""
        if 'tipo_custo_auto_tc' not in st.session_state:
            st.session_state.tipo_custo_auto_tc = "Variável"
        if 'usi_valor_tc' not in st.session_state:
            st.session_state.usi_valor_tc = ""
        
        # Função callback para atualizar campos quando Account mudar
        def atualizar_campos_account():
            account_atual = st.session_state.account_selectbox_tc
            if account_atual and account_atual != "Nenhum":
                # Verificar se já temos as informações em cache
                if account_atual in st.session_state.account_info_cache_tc:
                    cache_info = st.session_state.account_info_cache_tc[account_atual]
                    st.session_state.type06_valor_tc = cache_info.get('Type 06', '')
                    st.session_state.type05_valor_tc = cache_info.get('Type 05', '')
                    st.session_state.custo_valor_tc = cache_info.get('Custo', '')
                    st.session_state.usi_valor_tc = cache_info.get('USI', '')
                    st.session_state.tipo_custo_auto_tc = cache_info.get('Tipo_Custo', 'Variável')
                    
                    # Atualizar também os valores dos widgets diretamente
                    st.session_state.type06_display_tc = st.session_state.type06_valor_tc
                    st.session_state.type05_display_tc = st.session_state.type05_valor_tc
                    st.session_state.custo_display_tc = st.session_state.custo_valor_tc
                else:
                    ...
                    if df_para_buscar is not None and not df_para_buscar.empty:
                        ...
                        if info_account:
                            # Preencher valores
                            st.session_state.type06_valor_tc = info_account.get('Type 06', '')
                            st.session_state.type05_valor_tc = info_account.get('Type 05', '')
                            st.session_state.custo_valor_tc = info_account.get('Custo', '')
                            st.session_state.usi_valor_tc = info_account.get('USI', '')
                            ...
                            if 'Tipo_Custo' in info_account:
                                st.session_state.tipo_custo_auto_tc = info_account['Tipo_Custo']
                            else:
                                st.session_state.tipo_custo_auto_tc = "Variável"
                            
                            # Salvar no cache
                            st.session_state.account_info_cache_tc[account_atual] = {
                                'Type 06': st.session_state.type06_valor_tc,
                                'Type 05': st.session_state.type05_valor_tc,
                                'Custo': st.session_state.custo_valor_tc,
                                'USI': st.session_state.usi_valor_tc,
                                'Tipo_Custo': st.session_state.tipo_custo_auto_tc
                            }
                        else:
        ...
        if account_selecionado and account_selecionado != "Nenhum":
            ...
            account_anterior = st.session_state.get('account_anterior_tc', None)
            ...
            precisa_buscar = (
                account_mudou or
                account_selecionado not in st.session_state.account_info_cache_tc or
                not st.session_state.type06_valor_tc
            )
            
            if precisa_buscar:
                atualizar_campos_account()
                st.session_state.account_anterior_tc = account_selecionado
                # Forçar atualização da interface
                st.rerun()
        else:
            # Limpar campos se nenhum Account selecionado
            if st.session_state.type06_valor_tc or st.session_state.type05_valor_tc or st.session_state.custo_valor_tc:
                st.session_state.type06_valor_tc = ""
                st.session_state.type05_valor_tc = ""
                st.session_state.custo_valor_tc = ""
                st.session_state.tipo_custo_auto_tc = "Variável"
                st.session_state.account_anterior_tc = None
    ...
    with col_type1:
        ...
        if 'type06_display_tc' not in st.session_state:
            st.session_state.type06_display_tc = st.session_state.get('type06_valor_tc', '')
        else:
            # Atualizar o valor do widget se o valor mudou
            if st.session_state.get('type06_valor_tc', '') != st.session_state.type06_display_tc:
                st.session_state.type06_display_tc = st.session_state.get('type06_valor_tc', '')
        type06_display = st.text_input("Type 06:", value=st.session_state.type06_display_tc, key="type06_display_tc", disabled=True, help="Preenchido automaticamente ao selecionar Account")
    with col_type2:
        if 'type05_display_tc' not in st.session_state:
            st.session_state.type05_display_tc = st.session_state.get('type05_valor_tc', '')
        else:
            if st.session_state.get('type05_valor_tc', '') != st.session_state.type05_display_tc:
                st.session_state.type05_display_tc = st.session_state.get('type05_valor_tc', '')
        type05_display = st.text_input("Type 05:", value=st.session_state.type05_display_tc, key="type05_display_tc", disabled=True, help="Preenchido automaticamente ao selecionar Account")
    with col_type3:
        if 'custo_display_tc' not in st.session_state:
            st.session_state.custo_display_tc = st.session_state.get('custo_valor_tc', '')
        else:
            if st.session_state.get('custo_valor_tc', '') != st.session_state.custo_display_tc:
                st.session_state.custo_display_tc = st.session_state.get('custo_valor_tc', '')
        custo_display = st.text_input("Custo:", value=st.session_state.custo_display_tc, key="custo_display_tc", disabled=True, help="Preenchido automaticamente ao selecionar Account (determina Tipo_Custo: Fixo ou Variável)")
    
    # Mostrar mensagem se informações foram encontradas
    if account_selecionado and account_selecionado != "Nenhum" and st.session_state.type06_valor_tc:
        st.success(f"✅ Informações encontradas para Account '{account_selecionado}' - Tipo_Custo: {st.session_state.tipo_custo_auto_tc}")
            ...
            if not periodos_aplicar:
                ...
                for periodo in periodos_aplicar:
                    ...
                    for veiculo in veiculos_para_criar:
                        ...
                        novo_custo = {
                            ...
                            'Veículo': veiculo,
                            'Período': periodo,
                            'Custo': st.session_state.custo_valor_tc if st.session_state.custo_valor_tc else st.session_state.tipo_custo_auto_tc,
                            'Custo FP': valor_rateado,  # Valor já rateado para este veículo
                        ...
                        if st.session_state.type06_valor_tc:
                            novo_custo['Type 06'] = st.session_state.type06_valor_tc
                        if st.session_state.type05_valor_tc:
                            novo_custo['Type 05'] = st.session_state.type05_valor_tc
                        if st.session_state.custo_valor_tc:
                            novo_custo['Custo'] = st.session_state.custo_valor_tc
                        if st.session_state.usi_valor_tc:
                            novo_custo['USI'] = st.session_state.usi_valor_tc
                ...
                if linhas_novas:
                    ...
                    if salvar_custos_especificos(df_custos_especificos):
                        ...
                        if account_selecionado and account_selecionado != "Nenhum":
                            if account_selecionado in st.session_state.account_info_cache_tc:
                                del st.session_state.account_info_cache_tc[account_selecionado]
                        st.rerun()
                else:
...
if 'config_forecast_aplicada_tc' not in st.session_state:
    st.session_state.config_forecast_aplicada_tc = {
...
# ====================================================================
# 🧩 Referência por Modelo/Veículo (Realizado x Budget) — simples e opcional
# ====================================================================
# Mantém o comportamento atual por padrão (tudo Realizado). Quando um modelo
# estiver marcado como Budget, o BE usa Budget mês-a-mês (jan com jan, etc.).
try:
    if 'modelos_ref_budget_tc' not in st.session_state:
        st.session_state.modelos_ref_budget_tc = []

    modelos_ref_opcoes = []
    if 'df_filtrado' in locals() and df_filtrado is not None and 'Veículo' in df_filtrado.columns:
        modelos_ref_opcoes.extend(df_filtrado['Veículo'].dropna().astype(str).unique().tolist())
    # Incluir também veículos que só existam no Budget
    try:
        modelos_ref_opcoes.extend(get_veiculos_budget_opcoes(ultimo_ano_dados))
    except Exception:
        pass

    modelos_ref_opcoes = sorted({o.strip() for o in modelos_ref_opcoes if isinstance(o, str) and o.strip() and o.strip().lower() != 'todos'})

    with st.expander("🔁 Referência por modelo/veículo (Realizado x Budget)", expanded=False):
        st.caption("Padrão: Realizado (não altera o que já funciona). Se marcar um modelo, o BE passa a usar Budget mês-a-mês para esse modelo.")
        st.session_state.modelos_ref_budget_tc = st.multiselect(
            "Modelos com referência no Budget:",
            options=modelos_ref_opcoes,
            default=st.session_state.modelos_ref_budget_tc,
            help="Use quando o modelo não tem realizado suficiente, mas tem Budget. O cálculo compara mês com mês (jan com jan, etc.).",
        )

    config_forecast_temp['modelos_ref_budget_tc'] = st.session_state.modelos_ref_budget_tc
except Exception:
    # Se algo der errado aqui, não bloquear o simulador.
    config_forecast_temp['modelos_ref_budget_tc'] = st.session_state.get('modelos_ref_budget_tc', [])
...
if st.session_state.get('forecast_mensagem_sucesso_tc', None):
    st.markdown("---")
    st.success(st.session_state.forecast_mensagem_sucesso_tc)
    st.info(st.session_state.forecast_mensagem_info_tc)
    
    # 🔧 CORREÇÃO: Exibir também o log de processamento se existir
    if 'mensagens_debug_tc' in st.session_state and st.session_state.mensagens_debug_tc:
        mensagens_debug = st.session_state.mensagens_debug_tc.copy()  # Fazer cópia para não modificar a original
    ...
    st.session_state.forecast_mensagem_sucesso_tc = None
    st.session_state.forecast_mensagem_info_tc = None
...
if aplicar_config_forecast:
    ...
    st.session_state.config_forecast_aplicada_tc = config_forecast_temp.copy()
    
    # Salvar configurações de sensibilidade e inflação
    # Modo Global
    if config_sensibilidade_temp['sensibilidade_fixo'] is not None:
        st.session_state.sensibilidade_fixo_aplicada_tc = config_sensibilidade_temp['sensibilidade_fixo']
    if config_sensibilidade_temp['sensibilidade_variavel'] is not None:
        st.session_state.sensibilidade_variavel_aplicada_tc = config_sensibilidade_temp['sensibilidade_variavel']
    if config_sensibilidade_temp['inflacao_global'] is not None:
        st.session_state.inflacao_global_aplicada_tc = config_sensibilidade_temp['inflacao_global']
    
    # Modo Detalhado
    if config_sensibilidade_temp['sensibilidades_type06'] is not None:
        st.session_state.sensibilidades_aplicadas_tc = config_sensibilidade_temp['sensibilidades_type06']
    if config_sensibilidade_temp['inflacao_type06'] is not None:
        st.session_state.inflacao_aplicada_tc = config_sensibilidade_temp['inflacao_type06']
    ...
    modo_sel = st.session_state.get('modo_config_sensibilidade_tc', None)
    if modo_sel == 'global':
        st.session_state.sensibilidades_aplicadas_tc = None
        st.session_state.inflacao_aplicada_tc = None

    # Registrar o modo efetivamente aplicado
    st.session_state.modo_sensibilidade_aplicado_tc = modo_sel or st.session_state.get('modo_sensibilidade_aplicado_tc', 'global')
    ...
    try:
        caminho_forecast_vol = os.path.join("dados", "TC_Principal", "Forecast", "df_vol_historico.parquet")
        caminho_forecast_vol_excel = os.path.join("dados", "TC_Principal", "Forecast", "df_vol_historico.xlsx")
    ...
    def calcular_medias_forecast(df_filtrado_cache, colunas_adicionais_cache, periodos_para_media_cache, ultimo_periodo_dados_cache=None):
        ...
        if periodos_para_media_cache and 'Período' in df_filtrado_cache.columns:
            ...
            if not df_filtrado_media.empty:
                ...
                st.session_state['debug_calcular_medias_tc'] = {
        ...
        if df_filtrado_media.empty:
            ...
            df_medias = pd.DataFrame(columns=colunas_base + ['Custo FP'])
            colunas_media = ['Oficina', 'Veículo', 'Tipo_Custo'] + colunas_adicionais_cache
            df_media_mensal = pd.DataFrame(columns=colunas_media + ['Custo FP'])
        ...
        # 🔧 CORREÇÃO: Se houver coluna Ano, incluí-la no groupby (mesma lógica da TC_Principal)
    ...
    sensibilidade_fixo_aplicada = st.session_state.get('sensibilidade_fixo_aplicada_tc', None)
    sensibilidade_variavel_aplicada = st.session_state.get('sensibilidade_variavel_aplicada_tc', None)
    inflacao_global_aplicada = st.session_state.get('inflacao_global_aplicada_tc', None)
    sensibilidades_aplicadas = st.session_state.get('sensibilidades_aplicadas_tc', None)
    inflacao_aplicada = st.session_state.get('inflacao_aplicada_tc', None)
    
    # Obter configurações do forecast aplicadas
    config_forecast_aplicada = st.session_state.get('config_forecast_aplicada_tc', {})
    ...
    modelos_ref_budget = config_forecast_aplicada.get('modelos_ref_budget_tc', [])
    ...
    st.session_state.mensagens_debug_tc = []
    
    mensagens_debug = st.session_state.mensagens_debug_tc
    
    def adicionar_mensagem(tipo, mensagem):
        """Adiciona mensagem ao log de debug"""
        if 'mensagens_debug_tc' not in st.session_state:
            st.session_state.mensagens_debug_tc = []
        st.session_state.mensagens_debug_tc.append((tipo, mensagem))
    ...
    if modelos_ref_budget:
        adicionar_mensagem("info", f"   - Modelos referência Budget: {modelos_ref_budget}")
    ...
    with st.spinner("🔄 Gerando arquivos de forecast... Isso pode levar alguns minutos."):
        try:
            ...
            try:
                ...
                # Criar pasta Forecast dentro de dados/TC_Principal
            ...
            caminho_base_original = os.path.join("dados", "TC_Principal", "historico_consolidado", "df_principal_historico.parquet")
            ...
            if os.path.exists(caminho_base_original):
                ...
                caminho_forecast_original = os.path.join("dados", "TC_Principal", "historico_consolidado", "df_principal_historico_forecast.parquet")
                if os.path.exists(caminho_forecast_original):
                    ...
                    st.error("ℹ️ Por favor, verifique se o arquivo existe na pasta 'dados/TC_Principal/historico_consolidado/'")
                    st.stop()
            ...
            if periodos_para_media and 'Período' in df_base_filtrado.columns:
                ...
                if len(df_base_para_media) == 0:
                    # 🔧 CORREÇÃO: Se os períodos selecionados NÃO têm ano (ex: ["janeiro"])
                    # mas Período_Norm tem ano (ex: "janeiro 2026"), comparar só pelo mês.
                    selecao_sem_ano = all(
                        ' ' not in str(p).strip() or not str(p).strip().split(' ', 1)[1].isdigit()
                        for p in periodos_normalizados
                    )
                    if selecao_sem_ano:
                        periodos_so_mes = {str(p).strip().lower().split(' ', 1)[0] for p in periodos_normalizados}
                        df_base_filtrado_ano['_Mes_Norm_tmp'] = df_base_filtrado_ano['Período_Norm'].astype(str).str.split(' ', n=1).str[0]
                        df_base_para_media = df_base_filtrado_ano[df_base_filtrado_ano['_Mes_Norm_tmp'].isin(periodos_so_mes)].copy()
                        df_base_para_media = df_base_para_media.drop(columns=['_Mes_Norm_tmp'], errors='ignore')
                        df_base_filtrado_ano = df_base_filtrado_ano.drop(columns=['_Mes_Norm_tmp'], errors='ignore')
                        adicionar_mensagem("info", f"📊 Correspondência por mês (sem ano): {periodos_so_mes} → {len(df_base_para_media):,} linhas")
            ...
            # 🔧 CORREÇÃO: Sempre usar coluna 'Custo FP' (nunca 'Despesa Primaria')
            if 'Custo FP' not in df_base_para_media.columns:
                adicionar_mensagem("error", "❌ Coluna 'Custo FP' não encontrada nos dados!")
                st.error("❌ Coluna 'Custo FP' não encontrada nos dados!")
                st.stop()
            
            # 🔧 DEBUG: Verificar dados antes do cálculo da média
            total_antes_media = df_base_para_media['Custo FP'].sum()
            linhas_com_total_positivo = (df_base_para_media['Custo FP'] > 0).sum()
            linhas_com_total_zero = (df_base_para_media['Custo FP'] == 0).sum()
            ...
            adicionar_mensagem("info", f"   - Soma total de 'Custo FP': R$ {total_antes_media:,.2f}")
                    ...
                    if periodos_para_media and 'Período' in df_vol_historico.columns:
                        ...
                        if not df_vol_para_media.empty:
                            ...
                            if not df_vol_para_media.empty:
                                ...
                                # 🔧 CORREÇÃO CRÍTICA: Se o volume tem Veículo mas os dados de custo NÃO têm,
                                # devemos agregar os volumes de todos os veículos (soma) por período.
                                # Caso contrário, o merge falha e a proporção volume fica sempre 1.0,
                                # anulando completamente a sensibilidade (Fixo/Variável).
                                custo_tem_veiculo = 'Veículo' in df_base_filtrado.columns

                                if tem_veiculo_no_vol_para_media and not custo_tem_veiculo:
                                    # Volume tem Veículo mas custo NÃO → somar veículos por período
                                    # TC Veículos: volume de produção é COMUM para todas as oficinas,
                                    # variação existe apenas por modelo. Somamos para obter volume TOTAL.
                                    adicionar_mensagem("info", "📊 Volume tem Veículo mas custo não → somando volumes de todos os veículos por período (volume total de produção)")
                                    colunas_groupby_vol_medio = ['Período']
                                    chaves_volume_base = []
                                    # NÃO incluir Oficina no groupby — volume é igual para toda oficina
                                elif tem_veiculo_no_vol_para_media:
                                ...
                                # ETAPA 1: Agregar volume por período.
                                # Quando custo NÃO tem Veículo (TC Veículos), usar .sum()
                                # para obter volume TOTAL de produção (soma de todos os veículos).
                                # Quando custo TEM Veículo, usar .mean() (por veículo individual).
                                _vol_agg_fn = 'sum' if (tem_veiculo_no_vol_para_media and not custo_tem_veiculo) else 'mean'
                                df_vol_medio = df_vol_para_media.groupby(
                                    colunas_groupby_vol_medio,
                                    as_index=False,
                                    dropna=False,
                                )['Volume'].agg(_vol_agg_fn)
                            
                                # ETAPA 2: Calcular volume médio mensal (média dos períodos selecionados)
                                if chaves_volume_base:
                                    ...
                                    # Sem chaves de agrupamento → média geral de todos os períodos
                                    volume_base = pd.DataFrame({
                                        'Volume_Medio_Historico': [df_vol_medio['Volume'].mean()]
                                    })
                                adicionar_mensagem("info", f"📊 Volume_Medio_Historico calculado com agg='{_vol_agg_fn}': {volume_base['Volume_Medio_Historico'].iloc[0] if len(volume_base) > 0 else 'N/A':,.0f}")
                        ...
                        custo_tem_veiculo_fb = 'Veículo' in df_base_filtrado.columns

                        if tem_veiculo_no_vol_historico and not custo_tem_veiculo_fb:
                            # 🔧 Volume tem Veículo mas custo não → somar veículos por período, depois média
                            _gb_cols_sum = (['Oficina', 'Período'] if tem_oficina_fallback else ['Período'])
                            vol_total_periodo = df_vol_historico.groupby(_gb_cols_sum, as_index=False, dropna=False)['Volume'].sum()
                            _gb_cols_avg = (['Oficina'] if tem_oficina_fallback else [])
                            if _gb_cols_avg:
                                volume_base = vol_total_periodo.groupby(_gb_cols_avg, as_index=False, dropna=False)['Volume'].mean()
                            else:
                                volume_base = pd.DataFrame({'Volume': [vol_total_periodo['Volume'].mean()]})
                        elif tem_veiculo_no_vol_historico:
            ...
            modo_sens_aplicado = st.session_state.get('modo_sensibilidade_aplicado_tc', st.session_state.get('modo_config_sensibilidade_tc', 'global'))
            ...
            # ============================================================
            # 🆕 Volume BUD como fallback universal para sensibilidade
            # ============================================================
            # Quando volume real não está disponível para um período,
            # usar volume do Budget para criar variação significativa.
            # Sem isso, proporcao_volume = 1.0 e sensibilidade não tem efeito.
            _vol_bud_global = None
            try:
                _caminho_vol_bud = os.path.join('dados', 'TC_Principal', 'historico_consolidado', 'BUD', 'df_vol_historico_BUD.parquet')
                if os.path.exists(_caminho_vol_bud):
                    _vol_bud_global = pd.read_parquet(_caminho_vol_bud)
                    _vol_bud_global = _fix_mojibake_cols(_vol_bud_global)
                    for _c_txt in ['Oficina', 'Veículo', 'Período']:
                        if _c_txt in _vol_bud_global.columns:
                            _vol_bud_global[_c_txt] = _vol_bud_global[_c_txt].astype('string').str.strip()
                    if 'Ano' in _vol_bud_global.columns:
                        _vol_bud_global['Ano'] = pd.to_numeric(_vol_bud_global['Ano'], errors='coerce')
                    adicionar_mensagem("info", f"📊 Volume BUD fallback: {len(_vol_bud_global):,} linhas carregadas")
            except Exception:
                _vol_bud_global = None

            def _buscar_volume_bud_periodo(periodo_str_param, df_fc):
                """Tenta buscar volume BUD para um período específico.
                Retorna pd.Series (mapeado ao index de df_fc) ou None."""
                if _vol_bud_global is None or _vol_bud_global.empty:
                    return None
                if 'Volume' not in _vol_bud_global.columns:
                    return None
                try:
                    p_str = str(periodo_str_param).strip()
                    p_mes = p_str.split(' ', 1)[0].strip() if ' ' in p_str else p_str
                    p_ano = None
                    if ' ' in p_str:
                        ano_part = p_str.split(' ', 1)[1].strip()
                        if ano_part.isdigit():
                            p_ano = int(ano_part)

                    df_vb = _vol_bud_global.copy()
                    if 'Período' in df_vb.columns:
                        df_vb = df_vb[df_vb['Período'].astype(str).str.strip().str.capitalize() == p_mes.capitalize()]
                    if p_ano and 'Ano' in df_vb.columns:
                        df_vb = df_vb[df_vb['Ano'] == p_ano]
                    if df_vb.empty:
                        return None

                    _gb = []
                    if 'Oficina' in df_vb.columns:
                        _gb.append('Oficina')
                    # 🔧 CORREÇÃO: Só usar Veículo se existir tanto no volume BUD quanto nos dados de custo
                    usa_veic = 'Veículo' in df_vb.columns and df_vb['Veículo'].notna().any() and 'Veículo' in df_fc.columns
                    if usa_veic:
                        _gb.append('Veículo')

                    if _gb:
                        df_vb = df_vb.groupby(_gb, as_index=False, dropna=False)['Volume'].sum()
                        vol_dict = {tuple(str(r[c]) for c in _gb): float(r['Volume']) for _, r in df_vb.iterrows()}
                        vals = []
                        cnt_found = 0
                        for ix in df_fc.index:
                            chave = tuple(str(df_fc.loc[ix, c]) if c in df_fc.columns else '' for c in _gb)
                            if chave in vol_dict:
                                vals.append(vol_dict[chave])
                                cnt_found += 1
                            elif 'Volume_Medio_Historico' in df_fc.columns:
                                vals.append(float(df_fc.loc[ix, 'Volume_Medio_Historico']))
                            else:
                                vals.append(0.0)
                        if cnt_found > 0:
                            adicionar_mensagem("info", f"📊 Volume BUD fallback para '{periodo_str_param}': {cnt_found:,}/{len(df_fc):,} linhas mapeadas")
                            return pd.Series(vals, index=df_fc.index)
                    else:
                        vol_total = float(df_vb['Volume'].sum())
                        if vol_total > 0:
                            adicionar_mensagem("info", f"📊 Volume BUD fallback total para '{periodo_str_param}': {vol_total:,.0f}")
                            return pd.Series([vol_total] * len(df_fc), index=df_fc.index)
                    return None
                except Exception:
                    return None
            ...
            # ============================================================
            # 🆕 Budget como referência por modelo/veículo (opcional)
            # ============================================================
            modelos_ref_budget_set = {str(o).strip() for o in (modelos_ref_budget or []) if str(o).strip()}
            usa_budget_ref = None
            if modelos_ref_budget_set and 'Veículo' in df_forecast_completo.columns:
                usa_budget_ref = df_forecast_completo['Veículo'].astype('string').str.strip().isin(modelos_ref_budget_set)
            else:
            ...
            try:
                if modelos_ref_budget_set:
                    caminho_budget_custo = os.path.join('dados', 'TC_Principal', 'historico_consolidado', 'BUD', 'df_principal_historico_BUD.parquet')
                    caminho_budget_vol = os.path.join('dados', 'TC_Principal', 'historico_consolidado', 'BUD', 'df_vol_historico_BUD.parquet')
                    if os.path.exists(caminho_budget_custo) and os.path.exists(caminho_budget_vol):
                        ...
                        # Filtrar apenas modelos/veículos selecionados para Budget
                        if 'Veículo' in df_budget_custo.columns:
                            df_budget_custo = df_budget_custo[df_budget_custo['Veículo'].astype('string').str.strip().isin(modelos_ref_budget_set)].copy()
                        if 'Veículo' in df_budget_vol.columns:
                            df_budget_vol = df_budget_vol[df_budget_vol['Veículo'].astype('string').str.strip().isin(modelos_ref_budget_set)].copy()
                        ...
                        # ============================================================
                        # 🧩 Seed: modelos/veículos só no Budget (sem histórico)
                        # ============================================================
                        # Problema observado: alguns modelos/veículos existem no BUD (custo/volume)
                        # mas NÃO existem na base histórica df_principal_historico.parquet. Como o
                        # simulador parte do histórico para montar o df_forecast_completo, esses
                        # modelos nunca entram nos arquivos gerados (forecast_completo/
                        # df_principal_historico_forecast), resultando em gráficos vazios quando
                        # filtrados.
                        # Solução: quando o modelo foi marcado como "referência no Budget", criar
                        ...
                        try:
                            if (
                                modelos_ref_budget_set
                                and df_budget_custo is not None
                                and not df_budget_custo.empty
                                and df_forecast_completo is not None
                                and 'Veículo' in df_forecast_completo.columns
                            ):
                                modelos_no_historico = set(
                                    df_forecast_completo['Veículo']
                                    .astype('string')
                                    .str.strip()
                                    .dropna()
                                    .unique()
                                ...
                                modelos_sem_historico = [
                                    o for o in sorted(modelos_ref_budget_set)
                                    if o not in modelos_no_historico
                                ]

                                if modelos_sem_historico:
                                    df_seed = df_budget_custo[
                                        df_budget_custo['Veículo']
                                        .astype('string')
                                        .str.strip()
                                        .isin(modelos_sem_historico)
                                    ].copy()
                                    ...
                                    adicionar_mensagem(
                                        'success',
                                        f"✅ Budget ref: adicionadas {len(df_seed):,} linhas seed para modelos sem histórico: {', '.join(modelos_sem_historico)}",
                                    )
                        except Exception:
                            pass

                        # Montar dicts de custo do Budget em múltiplas granularidades (fallback)
                        if 'Custo FP' in df_budget_custo.columns and 'Oficina' in df_budget_custo.columns and 'Ano' in df_budget_custo.columns and '_MesNum' in df_budget_custo.columns and 'Custo' in df_budget_custo.columns:
                            ...
                            for cols_key_custo in candidatos_unicos:
                                df_bud_custo_g = df_budget_custo.groupby(cols_key_custo, dropna=False, as_index=False)['Custo FP'].sum()
                                d = {
                                    tuple(_norm_key_val(r[c]) for c in cols_key_custo): float(r['Custo FP'])
                        ...
                        adicionar_mensagem('warning', '⚠️ Budget ref: arquivos BUD não encontrados em dados/TC_Principal/historico_consolidado/BUD')
            except Exception:
                pass
            ...
            for periodo in periodos_restantes:
                ...
                if df_vol_por_mes is not None and not df_vol_por_mes.empty:
                    ...
                    if not vol_mes_df.empty:
                        ...
                        # 🔧 CORREÇÃO CRÍTICA: Só usar Veículo como chave se os dados de custo
                        # também têm Veículo. Caso contrário, somar todos os veículos (volume total).
                        usa_veiculo_no_mes = (
                        ...
                        # 🆕 CORREÇÃO: Tentar volume BUD antes de cair para Volume_Medio_Historico
                        _vol_bud_serie = _buscar_volume_bud_periodo(periodo, df_forecast_completo)
                        if _vol_bud_serie is not None:
                            volume_mes_serie = _vol_bud_serie
                            adicionar_mensagem("info", f"📊 Volume BUD usado como fallback para '{periodo}' (sensibilidade ativa)")
                        else:
                            adicionar_mensagem("warning", f"⚠️ Volume não encontrado para '{periodo}'. Usando volume médio histórico (sensibilidade neutra).")
                            if 'Volume_Medio_Historico' in df_forecast_completo.columns:
                ...
                # como linhas separadas no df_principal_historico_forecast
                ...
                for idx, custo_row in df_custos_especificos_para_forecast.iterrows():
                    ...
                    if periodos_aplicaveis:
                        ...
                        for periodo_aplicavel in periodos_aplicaveis:
                            ...
                            if 'Custo FP' not in linha_custo or pd.isna(linha_custo.get('Custo FP')):
                                if 'Despesa Primaria' in linha_custo and pd.notna(linha_custo.get('Despesa Primaria')):
                                    linha_custo['Custo FP'] = linha_custo['Despesa Primaria']
                                else:
                                    linha_custo['Custo FP'] = 0.0
                            
                            if 'Despesa Primaria' not in linha_custo or pd.isna(linha_custo.get('Despesa Primaria')):
                                if 'Custo FP' in linha_custo and pd.notna(linha_custo.get('Custo FP')):
                                    linha_custo['Despesa Primaria'] = linha_custo['Custo FP']
                                else:
                                    linha_custo['Despesa Primaria'] = 0.0
            ...
            def limpar_dataframe(df):
                ...
                if 'Custo FP' in df_limpo.columns:
                    mask_valor_valido = df_limpo['Custo FP'].notna() & (df_limpo['Custo FP'] != 0)
            ...
            if df_forecast_final is not None and not df_forecast_final.empty:
                ...
                if 'Custo FP' in df_forecast_final.columns:
                    valores_nao_zero = (df_forecast_final['Custo FP'].notna() & (df_forecast_final['Custo FP'] != 0)).sum()
            ...
            def padronizar_colunas(df, nome_tipo="DataFrame"):
                ...
                ordem_colunas_principal = [
                    'Oficina', 'Veículo', 'Ano', 'Período', 'Custo',
                    'Custo FP', 'Despesa Primaria',  # Total sempre antes de Valor
            ...
            try:
                ...
                caminho_vol_historico_original = os.path.join("dados", "TC_Principal", "historico_consolidado", "df_vol_historico.parquet")
            ...
            try:
                ...
                if os.path.exists(caminho_historico_salvo) and os.path.exists(caminho_forecast_salvo):
                    ...
                    caminho_consolidado_forecast = os.path.join(pasta_forecast, "df_principal_historico_forecast.parquet")
    ...
    st.session_state.gerar_tabela_completa_forecast_tc = True
    ...
    st.session_state.forecast_mensagem_sucesso_tc = "✅ **Configurações aplicadas com sucesso!**"
    st.session_state.forecast_mensagem_info_tc = "📊 **Arquivos de Best Estimate gerados!** Acesse a página 'Best Estimate (Análise)' para visualizar os gráficos e tabelas."

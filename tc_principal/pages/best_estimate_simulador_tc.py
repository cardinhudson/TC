...
def _salvar_config_forecast_tc(cfg: dict) -> None:
    """Persiste configurações de forecast em JSON."""
    os.makedirs(os.path.dirname(_CONFIG_FORECAST_TC_PATH), exist_ok=True)
    with open(_CONFIG_FORECAST_TC_PATH, 'w', encoding='utf-8') as f:
        json.dump(cfg, f, ensure_ascii=False, indent=2)


def _carregar_config_forecast_tc() -> dict | None:
    """Carrega configurações persistidas, se existirem."""
    if os.path.exists(_CONFIG_FORECAST_TC_PATH):
        with open(_CONFIG_FORECAST_TC_PATH, 'r', encoding='utf-8') as f:
            return json.load(f)
    return None
...
def encontrar_arquivo_parquet(nome_arquivo, ano_selecionado=None):
    ...
    if nome_arquivo == "df_principal.parquet":
        caminho_forecast = os.path.join(_DATA_ROOT, "TC_Principal", "Forecast", "forecast_completo.parquet")
        if os.path.exists(caminho_forecast):
            return caminho_forecast
    
    if nome_arquivo == "df_vol_veiculos.parquet":
        caminho_forecast_vol = os.path.join(_DATA_ROOT, "TC_Principal", "Forecast", "df_vol_historico.parquet")
    ...
    if ano_selecionado is not None and ano_selecionado != "Todos":
        caminho_ano = os.path.join(_DATA_ROOT, "TC_Principal", str(ano_selecionado), nome_arquivo)
        if os.path.exists(caminho_ano):
            return caminho_ano
    
    # 2. Tentar histórico consolidado (fallback)
    caminho_historico = os.path.join(_DATA_ROOT, "TC_Principal", "historico_consolidado", nome_arquivo.replace(".parquet", "_historico.parquet"))
    if os.path.exists(caminho_historico):
        return caminho_historico
    
    # TC Veículos: volume historico uses different naming
    if "vol" in nome_arquivo:
        caminho_hist_vol = os.path.join(_DATA_ROOT, "TC_Principal", "historico_consolidado", "df_vol_historico.parquet")
        if os.path.exists(caminho_hist_vol):
            return caminho_hist_vol
    
    # 3. Tentar pasta do ano mais recente
    pasta_dados = os.path.join(_DATA_ROOT, "TC_Principal")
    ...
    try:
        ...
        if arquivo_parquet is None:
            ...
            raise FileNotFoundError(
                "❌ Arquivo não encontrado: df_principal.parquet\n"
                "💡 Verifique se o arquivo existe em:\n"
                "   - dados/TC_Principal/Forecast/forecast_completo.parquet (PRIORIDADE)\n"
                "   - dados/TC_Principal/historico_consolidado/df_principal_historico.parquet\n"
                "   - dados/TC_Principal/{ANO}/df_principal.parquet"
            )
...
def load_volume_historico_data():
    ...
    try:
        ...
        caminho_forecast = os.path.join(_DATA_ROOT, "TC_Principal", "Forecast", "df_vol_historico.parquet")
        ...
        caminho_historico = os.path.join(_DATA_ROOT, "TC_Principal", "historico_consolidado", "df_vol_historico.parquet")
        if os.path.exists(caminho_historico):
...
# - Valores: dados/TC_Principal/Forecast/forecast_completo.parquet
# - Volume: dados/TC_Principal/Forecast/df_vol_historico.parquet
...
if 'Veículo' in df_visualizacao.columns and 'Período' in df_visualizacao.columns:
    ...
    # Fonte (canônica): dados/TC_Principal/Forecast/df_vol_historico.parquet (mesmo arquivo usado no forecast)
...
caminho_historico = os.path.join(_DATA_ROOT, "TC_Principal", "historico_consolidado", "df_principal_historico.parquet")
...
if 'Type 06' in df_filtrado.columns:
    ...
    if len(type06_valores) > 0:
        ...
        if modo_config == "🌐 Global (Fixo/Variável)":
            ...
            _cfg_p = _carregar_config_forecast_tc()
            if 'sensibilidade_fixo_aplicada_tc' not in st.session_state:
                st.session_state.sensibilidade_fixo_aplicada_tc = (_cfg_p or {}).get('sensibilidade_fixo', 0.0)
            if 'sensibilidade_variavel_aplicada_tc' not in st.session_state:
                st.session_state.sensibilidade_variavel_aplicada_tc = (_cfg_p or {}).get('sensibilidade_variavel', 1.0)
            if 'inflacao_global_aplicada_tc' not in st.session_state:
                st.session_state.inflacao_global_aplicada_tc = (_cfg_p or {}).get('inflacao_global', 0.0)
            if 'produtividade_global_aplicada_tc' not in st.session_state:
                st.session_state.produtividade_global_aplicada_tc = (_cfg_p or {}).get('produtividade_global', 0.0)
            ...
            with col_prod:
                produtividade_global_temp = st.slider(
                    ...
                    min_value=0.0,
                    max_value=20.0,
                    value=st.session_state.produtividade_global_aplicada_tc,
                    step=0.5,
                    format="%.2f",
            ...
            if 'produtividade_global_aplicada_tc' in st.session_state:
                produtividade_global = st.session_state.produtividade_global_aplicada_tc
            else:
            ...
            if 'valores_temp_prod_tc' not in st.session_state:
                st.session_state.valores_temp_prod_tc = {}
            ...
            with col_btn4:
                if st.button("🧹 Limpar Configurações", use_container_width=True, key="btn_limpar_tc"):
                    ...
                    st.session_state.produtividade_aplicada_tc = None
                    st.session_state.valores_temp_sens_tc = {}
                    st.session_state.valores_temp_infl_tc = {}
                    st.session_state.valores_temp_prod_tc = {}
            ...
            with col_infl_btn1:
                ...
                if st.button("📈 Aplicar", use_container_width=True, key="btn_aplicar_inflacao_tc"):
                    for type06 in type06_valores:
            ...
            with col_infl_btn2:
                st.markdown("<br>", unsafe_allow_html=True)
                if st.button("🔄 Zerar", use_container_width=True, key="btn_zerar_inflacao_tc"):
                    for type06 in type06_valores:
            ...
            with col_prod_input:
                prod_rapida = st.number_input(
                    ...
                    step=0.5,
                    format="%.2f",
                    key="prod_rapida_input_tc",
                    help="Ganho de produtividade que reduz o custo projetado"
                )
            
            with col_prod_btn1:
                st.markdown("<br>", unsafe_allow_html=True)
                if st.button("⚡ Aplicar", use_container_width=True, key="btn_aplicar_prod_tc"):
                    for type06 in type06_valores:
                        st.session_state.valores_temp_prod_tc[type06] = prod_rapida
                    st.session_state.widget_key_counter_tc += 1
                    st.rerun()
            
            with col_prod_btn2:
                st.markdown("<br>", unsafe_allow_html=True)
                if st.button("🔄 Zerar", use_container_width=True, key="btn_zerar_prod_tc"):
                    for type06 in type06_valores:
                        st.session_state.valores_temp_prod_tc[type06] = 0.0
                    st.session_state.widget_key_counter_tc += 1
                    st.rerun()
            ...
            **Produtividade**: Ganho de eficiência que reduz o custo projetado (ex: 5% = redução de 5%).
            ...
            for type06 in type06_valores:
                ...
                if type06 in st.session_state.valores_temp_prod_tc:
                    valor_padrao_prod = st.session_state.valores_temp_prod_tc[type06]
                ...
                with col7:
                    widget_key_prod = f"prod_{type06}_{st.session_state.widget_key_counter_tc}"
                    ...
                    st.session_state.valores_temp_prod_tc[type06] = produtividade
            ...
            if 'inflacao_aplicada_tc' not in st.session_state:
                st.session_state.inflacao_aplicada_tc = None
            if 'produtividade_aplicada_tc' not in st.session_state:
                st.session_state.produtividade_aplicada_tc = None
...
def carregar_custos_especificos():
    ...
    caminho_custos = os.path.join(_DATA_ROOT, "TC_Principal", "Forecast", "custos_especificos.parquet")
...
def salvar_custos_especificos(df):
    ...
    pasta_forecast = os.path.join(_DATA_ROOT, "TC_Principal", "Forecast")
...
def carregar_rateio_arquivo(ano_selecionado=None):
    ...
    try:
        ...
        if ano_selecionado and ano_selecionado != "Todos":
            caminho_rateio = os.path.join(_DATA_ROOT, str(ano_selecionado), "Reporting fluxo anexo.xlsx")
        else:
            # Tentar encontrar em qualquer pasta de ano
            caminho_rateio = None
            for ano in [2024, 2025]:
                caminho_teste = os.path.join(_DATA_ROOT, str(ano), "Reporting fluxo anexo.xlsx")
...
def buscar_info_por_account(account, df_base=None):
    ...
    if 'Custo' not in info:
        try:
            caminho_sapiens = os.path.join(_DATA_ROOT, "Dados SAPIENS.xlsx")
...
colunas_tabela_custos = [
    ...
    'Custo FP', 'Centocst', 'Fornec.', 'Fornecedor', 'USI',
...
if aplicar_config_forecast:
    ...
    if config_sensibilidade_temp.get('produtividade_global') is not None:
        st.session_state.produtividade_global_aplicada_tc = config_sensibilidade_temp['produtividade_global']
    ...
    _salvar_config_forecast_tc({
        ...
        'sensibilidade_variavel': st.session_state.get('sensibilidade_variavel_aplicada_tc', 1.0),
        'inflacao_global': st.session_state.get('inflacao_global_aplicada_tc', 0.0),
        'produtividade_global': st.session_state.get('produtividade_global_aplicada_tc', 0.0),
    ...
    try:
        caminho_forecast_vol = os.path.join(_DATA_ROOT, "TC_Principal", "Forecast", "df_vol_historico.parquet")
        caminho_forecast_vol_excel = os.path.join(_DATA_ROOT, "TC_Principal", "Forecast", "df_vol_historico.xlsx")
    ...
    def calcular_medias_forecast(df_filtrado_cache, colunas_adicionais_cache, periodos_para_media_cache, ultimo_periodo_dados_cache=None):
        ...
        if periodos_para_media_cache and 'Período' in df_filtrado_cache.columns:
            ...
            # NOTA: debug_calcular_medias_tc removido do cache (st.session_state proibido em @st.cache_data)
            # Para debug, verificar a saída do próprio DataFrame.
        else:
    ...
    with st.spinner("🔄 Gerando arquivos de forecast... Isso pode levar alguns minutos."):
        try:
            ...
            caminho_base_original = os.path.join(_DATA_ROOT, "TC_Principal", "historico_consolidado", "df_principal_historico.parquet")
            ...
            if os.path.exists(caminho_base_original):
                ...
                caminho_forecast_original = os.path.join(_DATA_ROOT, "TC_Principal", "historico_consolidado", "df_principal_historico_forecast.parquet")
            ...
            try:
                _caminho_vol_bud = os.path.join(_DATA_ROOT, 'TC_Principal', 'historico_consolidado', 'BUD', 'df_vol_historico_BUD.parquet')
            ...
            try:
                if modelos_ref_budget_set:
                    caminho_budget_custo = os.path.join(_DATA_ROOT, 'TC_Principal', 'historico_consolidado', 'BUD', 'df_principal_historico_BUD.parquet')
                    caminho_budget_vol = os.path.join(_DATA_ROOT, 'TC_Principal', 'historico_consolidado', 'BUD', 'df_vol_historico_BUD.parquet')
                ...
                for idx in df_forecast_completo.index:
                    ...
                    if _prod_t06_dict and 'Type 06' in df_forecast_completo.columns:
                        ...
                        if pd.notna(_t06_val) and _t06_val in _prod_t06_dict and _prod_t06_dict[_t06_val] > 0:
                            ...
                            produtividade_pct = (st.session_state.get('produtividade_global_aplicada_tc', 0.0) or 0.0) / 100.0
                    else:
                        produtividade_pct = (st.session_state.get('produtividade_global_aplicada_tc', 0.0) or 0.0) / 100.0
            ...
            try:
                ...
                caminho_vol_historico_original = os.path.join(_DATA_ROOT, "TC_Principal", "historico_consolidado", "df_vol_historico.parquet")
